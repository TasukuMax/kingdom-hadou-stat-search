const APP_DATA = {
  "characters": [
    {
      "id": 81,
      "name": "オルド",
      "rarity": "SSR",
      "tenpu": 900,
      "type": "援",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/546513",
      "sourceId": "546513",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_81.png",
      "battleArtName": "山城破り",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の対物を15％上昇させ、攻撃速度上昇20％と悠然を付与",
        "攻撃対象に対物250％の攻撃（自身の兵力が50％以上の場合、350％に威力上昇）"
      ],
      "battleArtType": "対物",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "対物攻撃に優れた武将",
          "悠然で敵のデバフを無効化"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "同金"
            },
            {
              "slot": "主将",
              "name": "オルド"
            },
            {
              "slot": "副将2",
              "name": "タジフ"
            },
            {
              "slot": "補佐1",
              "name": "ランカイ"
            },
            {
              "slot": "補佐2",
              "name": "シュンメン"
            }
          ],
          "selfSlot": "主将",
          "focusTitles": [
            "建物の耐久を削る編成",
            "前列で使うのがおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "北方山王伝",
              "effect": "習得した武将の戦法による対物上昇の効果×1.08 編制した部隊の対物+1% 習得した武将の攻撃+4"
            },
            {
              "name": "古豪英雄伝",
              "effect": "習得した武将の戦法による対物上昇の効果×1.07 編成した部隊の攻撃+1% 編成した部隊の対物+1%"
            },
            {
              "name": "城砦百伝・物",
              "effect": "編成した部隊の対物+1%"
            },
            {
              "name": "攻撃の鍛錬",
              "effect": "習得した武将の攻撃+5"
            },
            {
              "name": "戦威の鍛錬",
              "effect": "習得した武将の戦威+5"
            }
          ],
          "tipTitles": [
            "北方山王伝がおすすめ",
            "攻撃と戦威の鍛錬もおすすめ"
          ]
        }
      },
      "skills": [
        "猛者",
        "統制",
        "山読",
        "雄心"
      ],
      "personalities": [
        "防御",
        "函谷関の戦い",
        "燕軍",
        "武人",
        "慎重",
        "お調子者"
      ],
      "chainBase": 0.2,
      "attack": 712,
      "defense": 634,
      "war": 696,
      "strategy": 538
    },
    {
      "id": 5,
      "name": "バジオウ",
      "rarity": "SSR",
      "tenpu": 900,
      "type": "闘",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519318",
      "sourceId": "519318",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_5.png",
      "battleArtName": "死王の剣",
      "battleArtEffects": [
        "自部隊の攻撃を30％上昇させ会心発生上昇30％を付与",
        "攻撃対象の防御を20％低下させ攻撃対象と同じ横列の部隊に戦威250％の攻撃"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "戦法と会心で火力を出す武将",
          "主将や副将として活躍できる"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "バジオウ"
            },
            {
              "slot": "主将",
              "name": "輪虎"
            },
            {
              "slot": "副将2",
              "name": "介子坊"
            },
            {
              "slot": "補佐1",
              "name": "成恢"
            },
            {
              "slot": "補佐2",
              "name": "楚水"
            }
          ],
          "selfSlot": "副将1",
          "focusTitles": [
            "横列の敵部隊を撃破する編成",
            "技能で攻撃速度を上げて火力UP",
            "戦法で敵の耐久を下げる編成"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "獣威猛撃伝",
              "effect": "習得した武将の戦法威力×1.03 習得した武将の戦法による攻撃上昇の効果×1.06 習得した武将の攻撃+4"
            },
            {
              "name": "戦法技巧",
              "effect": "習得した武将の戦法威力×1.04"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            }
          ],
          "tipTitles": [
            "獣威猛撃伝がおすすめ",
            "攻撃や戦威を上げる秘伝と相性が良い"
          ]
        }
      },
      "skills": [
        "豪傑",
        "練武",
        "戦士",
        "扶助"
      ],
      "personalities": [
        "殲滅",
        "王弟反乱",
        "山の民",
        "忠誠心",
        "先陣をきる者",
        "武人"
      ],
      "chainBase": 0.2,
      "attack": 799,
      "defense": 755,
      "war": 738,
      "strategy": 435
    },
    {
      "id": 68,
      "name": "信(臨時千人将)",
      "rarity": "SSR",
      "tenpu": 900,
      "type": "援",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/533475",
      "sourceId": "533475",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_68.png",
      "battleArtName": "刹那の一撃",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の戦威を30％上昇させ、撃心発生上昇50％を付与",
        "攻撃対象に戦威250％の攻撃"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "早い",
      "guide": {
        "evaluationPoints": [
          "戦法で大ダメージを与える武将",
          "技能と個性も優秀"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "縛虎申"
            },
            {
              "slot": "主将",
              "name": "信(臨時千人将)"
            },
            {
              "slot": "副将2",
              "name": "楚水"
            },
            {
              "slot": "補佐1",
              "name": "録嗚未"
            },
            {
              "slot": "補佐2",
              "name": "漂"
            }
          ],
          "selfSlot": "主将",
          "focusTitles": [
            "戦法で撃心ダメージを与える編成",
            "副将は羌瘣や王賁もおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "勇往飛伝",
              "effect": "習得した武将の戦法による撃心発生上昇の効果時間+2秒 習得した武将の戦法に以下の効果を追加 ●自部隊の戦威+5%（効果時間2秒）"
            },
            {
              "name": "城砦千伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            }
          ],
          "tipTitles": [
            "勇往飛伝がおすすめ",
            "火力を伸ばせる秘伝がおすすめ"
          ]
        }
      },
      "skills": [
        "鋭鋒",
        "練武",
        "大勇",
        "不屈"
      ],
      "personalities": [
        "突撃",
        "山陽の戦い",
        "飛信隊",
        "大将軍への夢",
        "先陣をきる者",
        "一騎討ち"
      ],
      "chainBase": 0.2,
      "attack": 783,
      "defense": 686,
      "war": 712,
      "strategy": 390
    },
    {
      "id": 65,
      "name": "呉慶",
      "rarity": "SSR",
      "tenpu": 900,
      "type": "護",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/527706",
      "sourceId": "527706",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_65.png",
      "battleArtName": "九重槍璧",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の防御を40%上昇させ、反撃強化100%を付与（自身が主将の場合、反撃強化を軍勢全体に付与）",
        "自部隊に堅固5%を付与",
        "攻撃対象と同じ縦列の部隊に戦威200%の攻撃"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "遅い",
      "guide": {
        "evaluationPoints": [
          "反撃で大ダメージを与える武将",
          "主将で使うのがおすすめ"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "宮元"
            },
            {
              "slot": "主将",
              "name": "呉慶"
            },
            {
              "slot": "副将2",
              "name": "黄離弦"
            },
            {
              "slot": "補佐1",
              "name": "魏加"
            },
            {
              "slot": "補佐2",
              "name": "河了貂"
            }
          ],
          "selfSlot": "主将",
          "focusTitles": [
            "反撃で大ダメージを与える編成",
            "軍勢の後列で使うのがおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "圧潰撃滅伝",
              "effect": "習得した武将の戦法による防御上昇の効果×1.05 編成した部隊の攻撃+1% 編成した部隊の防御+1%"
            },
            {
              "name": "城砦千伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "戦法技巧",
              "effect": "習得した武将の戦法威力×1.04"
            },
            {
              "name": "城砦百伝・防",
              "effect": "編成した部隊の防御+1%"
            },
            {
              "name": "城砦伍伝・防",
              "effect": "編成した部隊の防御+0.4%"
            }
          ],
          "tipTitles": [
            "圧潰撃滅伝がおすすめ",
            "秘伝で攻撃か防御を強化しよう"
          ]
        }
      },
      "skills": [
        "頑健",
        "開智",
        "報復",
        "後備"
      ],
      "personalities": [
        "殲滅",
        "蛇甘平原の戦い",
        "魏軍",
        "戦術眼",
        "王の一族",
        "一騎討ち"
      ],
      "chainBase": 0.2,
      "attack": 729,
      "defense": 799,
      "war": 783,
      "strategy": 747
    },
    {
      "id": 89,
      "name": "呉鳳明",
      "rarity": "SSR",
      "tenpu": 900,
      "type": "護",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/549059",
      "sourceId": "549059",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_89.png",
      "battleArtName": "流動力術",
      "battleArtEffects": [
        "自部隊に悠然を付与",
        "自部隊と同じ横列の部隊の防御、策略、対物を35％上昇させ、反撃強化50％を付与",
        "攻撃対象と同じ縦列の部隊に戦威200％の攻撃"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "耐久と対物に優れた武将",
          "確定入手にはガチャ200連が必要"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "漂"
            },
            {
              "slot": "主将",
              "name": "王翦"
            },
            {
              "slot": "副将2",
              "name": "呉鳳明"
            },
            {
              "slot": "補佐1",
              "name": "隆国"
            },
            {
              "slot": "補佐2",
              "name": "介子坊"
            }
          ],
          "selfSlot": "副将2",
          "focusTitles": [
            "軍団戦において活躍する編成",
            "前列で使うのがおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "攻城打尽伝",
              "effect": "習得した武将の戦法威力×1.03 習得した武将の戦法による防御上昇の効果×1.04 習得した武将の戦法に以下の効果を追加 ●自部隊の対物+5%（効果時間2秒）"
            },
            {
              "name": "対物上昇の激魂",
              "effect": "習得した武将の戦法による対物上昇の効果×1.02 習得した武将の戦法による対物上昇の効果時間+2秒"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            }
          ],
          "tipTitles": [
            "攻城打尽伝がおすすめ",
            "対物上昇の激魂もおすすめ"
          ]
        }
      },
      "skills": [
        "聡明",
        "開智",
        "堅靭",
        "攻城"
      ],
      "personalities": [
        "防御",
        "函谷関の戦い",
        "魏軍",
        "戦術眼",
        "器用",
        "慎重"
      ],
      "chainBase": 0.2,
      "attack": 677,
      "defense": 755,
      "war": 642,
      "strategy": 783
    },
    {
      "id": 6,
      "name": "嬴政",
      "rarity": "SSR",
      "tenpu": 900,
      "type": "護",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519317",
      "sourceId": "519317",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_6.png",
      "battleArtName": "不撓不屈",
      "battleArtEffects": [
        "自部隊と同じ縦列の部隊の防御、戦威、策略を20％上昇させ、堅固5％を付与"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "早い",
      "guide": {
        "evaluationPoints": [
          "戦法で味方を強化する武将",
          "技能で部隊の攻撃と防御を上昇"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "嬴政"
            },
            {
              "slot": "主将",
              "name": "摎"
            },
            {
              "slot": "副将2",
              "name": "羌瘣"
            },
            {
              "slot": "補佐1",
              "name": "漂"
            },
            {
              "slot": "補佐2",
              "name": "紫夏"
            }
          ],
          "selfSlot": "副将1",
          "focusTitles": [
            "戦法で大ダメージを与える編成",
            "雄心で攻撃と防御を15％UP",
            "反撃で敵にダメージを与える編成"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "戦威上昇の激心",
              "effect": "習得した武将の戦法による戦威上昇の効果×1.06"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            },
            {
              "name": "減戦短縮の印",
              "effect": "習得した武将の戦威+4"
            }
          ],
          "tipTitles": [
            "戦威上昇の激心がおすすめ",
            "終乱堅志伝は主将時におすすめ"
          ]
        }
      },
      "skills": [
        "威風",
        "統制",
        "雄心",
        "堅実"
      ],
      "personalities": [
        "主導",
        "王弟反乱",
        "大王一派",
        "王の一族",
        "中華統一",
        "戦術眼"
      ],
      "chainBase": 0.2,
      "attack": 660,
      "defense": 651,
      "war": 703,
      "strategy": 729
    },
    {
      "id": 79,
      "name": "廉頗",
      "rarity": "SSR",
      "tenpu": 900,
      "type": "援",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/541917",
      "sourceId": "541917",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_79.png",
      "battleArtName": "天地激震",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の",
        "攻撃を30%、対物を60%上昇",
        "（自身が主将の場合、対物上昇を軍勢全体に付与）",
        "攻撃対象に対物300%の攻撃"
      ],
      "battleArtType": "対物",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "対物攻撃に特化した援タイプ武将",
          "主将適性が非常に高い"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "姜燕"
            },
            {
              "slot": "主将",
              "name": "廉頗"
            },
            {
              "slot": "副将2",
              "name": "介子坊"
            },
            {
              "slot": "補佐1",
              "name": "ランカイ"
            },
            {
              "slot": "補佐2",
              "name": "同金"
            }
          ],
          "selfSlot": "主将",
          "focusTitles": [
            "敵の建物に大ダメージを与える編成",
            "軍勢の前列で使うのがおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "古豪英雄伝",
              "effect": "習得した武将の戦法による対物上昇の効果×1.07 編成した部隊の攻撃+1% 編成した部隊の対物+1%"
            },
            {
              "name": "城砦百伝・物",
              "effect": "編成した部隊の対物+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "攻撃の鍛錬",
              "effect": "習得した武将の攻撃+5"
            },
            {
              "name": "攻防の鍛錬",
              "effect": "習得した武将の攻撃+3 習得した武将の防御+3"
            }
          ],
          "tipTitles": [
            "古豪英雄伝がおすすめ",
            "城砦百伝・物もおすすめ"
          ]
        }
      },
      "skills": [
        "威風",
        "洞察",
        "伝熱",
        "尖撃"
      ],
      "personalities": [
        "主導",
        "山陽の戦い",
        "廉頗軍",
        "戦術眼",
        "激情家",
        "一騎討ち"
      ],
      "chainBase": 0.2,
      "attack": 808,
      "defense": 799,
      "war": 825,
      "strategy": 738
    },
    {
      "id": 77,
      "name": "摎",
      "rarity": "SSR",
      "tenpu": 900,
      "type": "闘",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/537926",
      "sourceId": "537926",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_77.png",
      "battleArtName": "破城掃滅",
      "battleArtEffects": [
        "自部隊に攻撃速度上昇20％、会心発生上昇30％を付与",
        "攻撃対象と同じ縦列の部隊に戦威250％の攻撃",
        "（敵部隊の戦威より自部隊の戦威が上回っている場合、300％に威力上昇）"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "攻撃的な戦法を持つ闘タイプ武将",
          "技能で軍勢の機動が上昇する"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "嬴政"
            },
            {
              "slot": "主将",
              "name": "摎"
            },
            {
              "slot": "副将2",
              "name": "羌瘣"
            },
            {
              "slot": "補佐1",
              "name": "漂"
            },
            {
              "slot": "補佐2",
              "name": "宮元"
            }
          ],
          "selfSlot": "主将",
          "focusTitles": [
            "戦法で敵の兵力を削る編成",
            "前列で使うのがおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "烈火戦姫伝",
              "effect": "習得した武将の戦法による攻撃速度上昇の効果時間+2秒 編成した部隊の攻撃+1% 編成した部隊の戦威+1%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            },
            {
              "name": "城砦伍伝・戦策",
              "effect": "編成した部隊の戦威+0.2% 編成した部隊の策略+0.2%"
            },
            {
              "name": "戦威の鍛錬",
              "effect": "習得した武将の戦威+5"
            }
          ],
          "tipTitles": [
            "烈火戦姫伝がおすすめ",
            "戦威上昇の秘伝と相性が良い"
          ]
        }
      },
      "skills": [
        "雄心",
        "練武",
        "戦神子",
        "攻陣"
      ],
      "personalities": [
        "主導",
        "一騎討ち",
        "王の一族",
        "戦術眼",
        "先陣をきる者",
        "剣客"
      ],
      "chainBase": 0.2,
      "attack": 808,
      "defense": 677,
      "war": 808,
      "strategy": 755
    },
    {
      "id": 83,
      "name": "李牧",
      "rarity": "SSR",
      "tenpu": 900,
      "type": "妨",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/546515",
      "sourceId": "546515",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_83.png",
      "battleArtName": "一望千里",
      "battleArtEffects": [
        "攻撃対象と同じ縦列の部隊の強化効果を2つ解除し、攻撃を20％、策略を40％低下させ、恐怖を付与し、戦威200％の攻撃"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "早い",
      "guide": {
        "evaluationPoints": [
          "敵の妨害に特化した武将",
          "共通する個性を持つ武将が少ない"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "羌瘣"
            },
            {
              "slot": "主将",
              "name": "李牧"
            },
            {
              "slot": "副将2",
              "name": "宮元"
            },
            {
              "slot": "補佐1",
              "name": "隆国"
            },
            {
              "slot": "補佐2",
              "name": "成恢"
            }
          ],
          "selfSlot": "主将",
          "focusTitles": [
            "複数のデバフで敵を妨害する編成",
            "交戦開始時からデバフを付与"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "天智之導伝",
              "effect": "習得した武将の戦法威力×1.03 習得した武将の戦法に以下の効果を追加 ●自部隊の策略+5%（効果時間2秒） ●攻撃対象の部隊の策略₋2%（効果時間6秒）"
            },
            {
              "name": "城砦千伝・策",
              "effect": "編成した部隊の策略+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            },
            {
              "name": "城砦伍伝・策",
              "effect": "編成した部隊の策略+0.4%"
            }
          ],
          "tipTitles": [
            "天智之導伝がおすすめ",
            "城砦千伝・策もおすすめ"
          ]
        }
      },
      "skills": [
        "掃討",
        "開智",
        "才賢",
        "俊才"
      ],
      "personalities": [
        "主導",
        "函谷関の戦い",
        "趙軍",
        "李牧一派",
        "戦術眼",
        "奇策奇計"
      ],
      "chainBase": 0.2,
      "attack": 609,
      "defense": 712,
      "war": 686,
      "strategy": 860
    },
    {
      "id": 2,
      "name": "楊端和",
      "rarity": "SSR",
      "tenpu": 900,
      "type": "妨",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519321",
      "sourceId": "519321",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_2.png",
      "battleArtName": "山界の死王",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊に攻撃速度上昇20%を付与",
        "攻撃対象と同じ横列の部隊に恐怖を付与",
        "攻撃対象に戦威200%の攻撃"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "恐怖で敵部隊を妨害する武将",
          "戦法と会心の両方で火力を出せる"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "バジオウ"
            },
            {
              "slot": "主将",
              "name": "楊端和"
            },
            {
              "slot": "副将2",
              "name": "徐完"
            },
            {
              "slot": "補佐1",
              "name": "陸仙"
            },
            {
              "slot": "補佐2",
              "name": "干央"
            }
          ],
          "selfSlot": "主将",
          "focusTitles": [
            "戦法と会心で火力を出す編成",
            "S3は中列で使うのがおすすめ",
            "戦法と会心で火力を出す編成"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "疾風死王伝",
              "effect": "習得した武将の戦法威力×1.04 習得した武将の戦法による攻撃速度上昇の効果時間+2秒"
            },
            {
              "name": "城砦千伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "戦法技巧",
              "effect": "習得した武将の戦法威力×1.04"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            }
          ],
          "tipTitles": [
            "疾風死王伝がおすすめ",
            "副将なら城砦千伝・攻がおすすめ"
          ]
        }
      },
      "skills": [
        "鋭鋒",
        "練武",
        "死王",
        "枢機"
      ],
      "personalities": [
        "殲滅",
        "王弟反乱",
        "山の民",
        "中華統一",
        "先陣をきる者",
        "剣客"
      ],
      "chainBase": 0.2,
      "attack": 825,
      "defense": 834,
      "war": 808,
      "strategy": 686
    },
    {
      "id": 82,
      "name": "汗明",
      "rarity": "SSR",
      "tenpu": 900,
      "type": "闘",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/546514",
      "sourceId": "546514",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_82.png",
      "battleArtName": "至強の打擲",
      "battleArtEffects": [
        "自部隊の防御を20％上昇させ、会心発生上昇30％を付与",
        "攻撃対象と同じ横列の部隊に戦威300％の攻撃（自部隊に付与されている強化効果の種類×10%威力上昇。10種類まで計上し、効果最大で100％上昇して400％に威力上昇）"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "遅い",
      "guide": {
        "evaluationPoints": [
          "戦法で大ダメージを与える武将",
          "通常攻撃でも火力を出せる"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "臨武君"
            },
            {
              "slot": "主将",
              "name": "汗明"
            },
            {
              "slot": "副将2",
              "name": "介子坊"
            },
            {
              "slot": "補佐1",
              "name": "徐完"
            },
            {
              "slot": "補佐2",
              "name": "ムタ"
            }
          ],
          "selfSlot": "主将",
          "focusTitles": [
            "敵軍勢に大ダメージを与える編成",
            "前列で使うのがおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "至強楚伝",
              "effect": "習得した武将の戦法威力×1.06 編成した部隊の攻撃+0.3% 編成した部隊の防御+0.3%"
            },
            {
              "name": "激炎豪昇伝",
              "effect": "習得した武将の戦法威力×1.03 習得した武将の戦法による会心発生上昇の効果時間+2秒 習得した武将の戦威+4"
            },
            {
              "name": "会心上昇の炎",
              "effect": "習得した武将の戦法による会心発生上昇の効果時間+0秒 習得した武将の攻撃+4"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            }
          ],
          "tipTitles": [
            "至強楚伝がおすすめ",
            "会心上昇の炎もおすすめ"
          ]
        }
      },
      "skills": [
        "尖撃",
        "練武",
        "巨人",
        "大勇"
      ],
      "personalities": [
        "主導",
        "函谷関の戦い",
        "楚軍",
        "武人",
        "怪力",
        "一騎討ち"
      ],
      "chainBase": 0.2,
      "attack": 842,
      "defense": 677,
      "war": 703,
      "strategy": 547
    },
    {
      "id": 80,
      "name": "王翦",
      "rarity": "SSR",
      "tenpu": 900,
      "type": "護",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/546512",
      "sourceId": "546512",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_80.png",
      "battleArtName": "千考掌術",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の攻撃を20％、防御を40％上昇させて反撃強化50％を付与",
        "攻撃対象と同じ横列の部隊に戦慄3％を付与して戦威200％の攻撃"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "遅い",
      "guide": {
        "evaluationPoints": [
          "対軍勢の戦闘で活躍する武将",
          "戦法の戦慄が強力"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "宮元"
            },
            {
              "slot": "主将",
              "name": "王翦"
            },
            {
              "slot": "副将2",
              "name": "漂"
            },
            {
              "slot": "補佐1",
              "name": "干央"
            },
            {
              "slot": "補佐2",
              "name": "バミュウ"
            }
          ],
          "selfSlot": "主将",
          "focusTitles": [
            "戦法で敵部隊を妨害する編成",
            "前列で使うのがおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "大望之策伝",
              "effect": "習得した武将の戦法による防御上昇の効果×1.07 習得した武将の戦法による反撃強化の効果×1.07 編成した部隊に付与された策略低下の効果時間₋2秒"
            },
            {
              "name": "圧潰撃滅伝",
              "effect": "習得した武将の戦法による防御上昇の効果×1.05 編成した部隊の攻撃+1% 編成した部隊の防御+1%"
            },
            {
              "name": "戦法技巧",
              "effect": "習得した武将の戦法威力×1.04"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            }
          ],
          "tipTitles": [
            "大望之策伝がおすすめ",
            "火力を伸ばせる戦法技巧もあり"
          ]
        }
      },
      "skills": [
        "聡明",
        "洞察",
        "不敗",
        "衆兵"
      ],
      "personalities": [
        "防御",
        "函谷関の戦い",
        "王翦軍",
        "戦術眼",
        "野心家",
        "暗躍"
      ],
      "chainBase": 0.2,
      "attack": 747,
      "defense": 755,
      "war": 668,
      "strategy": 834
    },
    {
      "id": 70,
      "name": "王賁",
      "rarity": "SSR",
      "tenpu": 900,
      "type": "闘",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/533477",
      "sourceId": "533477",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_70.png",
      "battleArtName": "龍指",
      "battleArtEffects": [
        "自部隊の攻撃を20％上昇",
        "攻撃対象と同じ縦列の部隊に戦威250％の攻撃（攻撃対象には350％の攻撃）",
        "攻撃対象と同じ縦列の部隊の防御を40％低下"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "敵部隊に大ダメージを与える武将",
          "戦威バフや撃心発生を補うと強力"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "関常"
            },
            {
              "slot": "主将",
              "name": "王賁"
            },
            {
              "slot": "副将2",
              "name": "介子坊"
            },
            {
              "slot": "補佐1",
              "name": "臨武君"
            },
            {
              "slot": "補佐2",
              "name": "姜燕"
            }
          ],
          "selfSlot": "主将",
          "focusTitles": [
            "戦威ダメージで敵を撃破する編成",
            "軍勢の前列で使うのがおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "穿鳳伝",
              "effect": "習得した武将の戦法による防御低下の効果時間+2秒 習得した武将の戦法に以下の効果を追加 ●自部隊の攻撃+5%（効果時間2秒）"
            },
            {
              "name": "城砦千伝・戦",
              "effect": "編成した部隊の戦威+1%"
            },
            {
              "name": "城砦千伝・攻戦",
              "effect": "編成した部隊の攻撃+1% 編成した部隊の戦威+1%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            },
            {
              "name": "防御低下の炎",
              "effect": "習得した武将の戦法による防御低下の効果時間+0秒 習得した武将の戦威+4"
            }
          ],
          "tipTitles": [
            "穿鳳伝がおすすめ",
            "戦威上昇の秘伝と相性が良い"
          ]
        }
      },
      "skills": [
        "尖撃",
        "統制",
        "俊才",
        "攻陣"
      ],
      "personalities": [
        "主導",
        "山陽の戦い",
        "玉鳳隊",
        "槍名人",
        "大将軍への夢",
        "武人"
      ],
      "chainBase": 0.2,
      "attack": 677,
      "defense": 642,
      "war": 764,
      "strategy": 538
    },
    {
      "id": 1,
      "name": "王騎",
      "rarity": "SSR",
      "tenpu": 900,
      "type": "援",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519322",
      "sourceId": "519322",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_1.png",
      "battleArtName": "全軍前進",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の攻撃、防御を15％上昇（自部隊が前列にいる場合、軍勢全体に付与）",
        "攻撃対象と同じ横列の部隊に戦威200％の攻撃"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "遅い",
      "guide": {
        "evaluationPoints": [
          "戦法で軍勢全体を強化する武将",
          "技能で闘タイプ武将としても扱う"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "騰"
            },
            {
              "slot": "主将",
              "name": "王騎"
            },
            {
              "slot": "副将2",
              "name": "羌瘣"
            },
            {
              "slot": "補佐1",
              "name": "姜燕"
            },
            {
              "slot": "補佐2",
              "name": "楚水"
            }
          ],
          "selfSlot": "主将",
          "focusTitles": [
            "戦法で敵部隊を撃破する編成",
            "軍勢の前列で使うのがおすすめ",
            "軍勢の前列で活躍する編成"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "怪鳥飛翔伝",
              "effect": "習得した武将の戦法威力×1.03 習得した武将の戦法による攻撃上昇の効果時間+2秒 習得した武将の攻撃+4"
            },
            {
              "name": "城砦千伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            }
          ],
          "tipTitles": [
            "怪鳥飛翔伝がおすすめ",
            "城砦百伝・攻や城砦伍伝・攻もあり"
          ]
        }
      },
      "skills": [
        "威風",
        "統制",
        "怪鳥",
        "尖撃"
      ],
      "personalities": [
        "主導",
        "馬陽の戦い",
        "王騎軍",
        "中華統一",
        "一騎討ち",
        "戦術眼"
      ],
      "chainBase": 0.2,
      "attack": 842,
      "defense": 842,
      "war": 825,
      "strategy": 755
    },
    {
      "id": 7,
      "name": "羌瘣",
      "rarity": "SSR",
      "tenpu": 900,
      "type": "妨",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519316",
      "sourceId": "519316",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_7.png",
      "battleArtName": "蚩尤の巫舞",
      "battleArtEffects": [
        "自部隊の攻撃、策略を30％上昇",
        "攻撃対象と同じ縦列の部隊に攻撃速度低下20％を付与し、戦威200％の攻撃（攻撃対象には300％の攻撃）"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "早い",
      "guide": {
        "evaluationPoints": [
          "攻撃と妨害が得意な武将",
          "あらゆる部隊の副将で使える"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "嬴政"
            },
            {
              "slot": "主将",
              "name": "摎"
            },
            {
              "slot": "副将2",
              "name": "羌瘣"
            },
            {
              "slot": "補佐1",
              "name": "漂"
            },
            {
              "slot": "補佐2",
              "name": "紫夏"
            }
          ],
          "selfSlot": "副将2",
          "focusTitles": [
            "戦法で大ダメージを与える編成",
            "前列で使うのがおすすめ",
            "戦法で敵部隊を撃破する編成"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "巫舞堕神伝",
              "effect": "習得した武将の戦法威力×1.02 習得した武将の戦法による攻撃速度低下の効果×1.02 習得した武将の戦法による攻撃速度低下の効果時間+2秒"
            },
            {
              "name": "城砦千伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦千伝・策",
              "effect": "編成した部隊の策略+1%"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            }
          ],
          "tipTitles": [
            "巫舞堕神伝がおすすめ",
            "巫舞堕神伝は副将でも使える"
          ]
        }
      },
      "skills": [
        "聡明",
        "開智",
        "攻陣",
        "助勢"
      ],
      "personalities": [
        "突撃",
        "蛇甘平原の戦い",
        "戦術眼",
        "暗躍",
        "一騎討ち",
        "先陣をきる者"
      ],
      "chainBase": 0.2,
      "attack": 764,
      "defense": 651,
      "war": 747,
      "strategy": 529
    },
    {
      "id": 69,
      "name": "蒙恬",
      "rarity": "SSR",
      "tenpu": 900,
      "type": "護",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/533476",
      "sourceId": "533476",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_69.png",
      "battleArtName": "活路への布石",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の攻撃を20％上昇させ、確率回避50％、会心発生上昇30％を付与"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "通常攻撃を避ける武将",
          "摎や嬴政と同じ技能を持つ"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "紫夏"
            },
            {
              "slot": "主将",
              "name": "蒙恬"
            },
            {
              "slot": "副将2",
              "name": "陸仙"
            },
            {
              "slot": "補佐1",
              "name": "嬴政"
            },
            {
              "slot": "補佐2",
              "name": "成恢"
            }
          ],
          "selfSlot": "主将",
          "focusTitles": [
            "反撃と会心で敵を撃破する編成",
            "軍勢内のどの列でも使える"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "会心上昇の炎",
              "effect": "習得した武将の戦法による会心発生上昇の効果時間+0秒 習得した武将の攻撃+4"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "減攻短縮の印",
              "effect": "習得した武将の攻撃+4"
            }
          ],
          "tipTitles": [
            "城砦百伝・攻がおすすめ",
            "飄烈自在伝は主将時におすすめ"
          ]
        }
      },
      "skills": [
        "猛者",
        "開智",
        "雄心",
        "枢機"
      ],
      "personalities": [
        "主導",
        "山陽の戦い",
        "楽華隊",
        "戦術眼",
        "大将軍への夢",
        "器用"
      ],
      "chainBase": 0.2,
      "attack": 625,
      "defense": 696,
      "war": 581,
      "strategy": 721
    },
    {
      "id": 76,
      "name": "蒙驁",
      "rarity": "SSR",
      "tenpu": 900,
      "type": "護",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/536784",
      "sourceId": "536784",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_76.png",
      "battleArtName": "常勝戦術",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の弱化効果を3つ解除し、防御を40％、戦威を20％上昇させ、負傷兵を最大兵力の20％回復",
        "自部隊に反撃強化50％を付与"
      ],
      "battleArtType": "回復",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "耐久力に優れた護タイプ武将",
          "主に麃公の副将で使える"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "蒙驁"
            },
            {
              "slot": "主将",
              "name": "麃公"
            },
            {
              "slot": "副将2",
              "name": "縛虎申"
            },
            {
              "slot": "補佐1",
              "name": "姜燕"
            },
            {
              "slot": "補佐2",
              "name": "楚水"
            }
          ],
          "selfSlot": "副将1",
          "focusTitles": [
            "高威力の戦威ダメージを与える編成",
            "蒙驁は副将1に編成する"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦千伝・攻戦",
              "effect": "編成した部隊の攻撃+1% 編成した部隊の戦威+1%"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            }
          ],
          "tipTitles": [
            "城砦千伝・攻戦がおすすめ",
            "白老伝は条件を満たせるなら強力"
          ]
        }
      },
      "skills": [
        "頑健",
        "人望",
        "白老",
        "守壁"
      ],
      "personalities": [
        "防御",
        "山陽の戦い",
        "経験豊富",
        "怪力",
        "一騎討ち",
        "基本戦術"
      ],
      "chainBase": 0.2,
      "attack": 668,
      "defense": 783,
      "war": 642,
      "strategy": 660
    },
    {
      "id": 67,
      "name": "輪虎",
      "rarity": "SSR",
      "tenpu": 900,
      "type": "妨",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/533474",
      "sourceId": "533474",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_67.png",
      "battleArtName": "天与の剣",
      "battleArtEffects": [
        "自部隊に攻撃拡張・横を付与",
        "攻撃対象と同じ横列の部隊の攻撃、防御、戦威を25％低下させ、戦威200％の攻撃"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "通常攻撃で横列にダメージを与える",
          "対軍勢の戦闘において優秀"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "バジオウ"
            },
            {
              "slot": "主将",
              "name": "輪虎"
            },
            {
              "slot": "副将2",
              "name": "介子坊"
            },
            {
              "slot": "補佐1",
              "name": "成恢"
            },
            {
              "slot": "補佐2",
              "name": "楚水"
            }
          ],
          "selfSlot": "主将",
          "focusTitles": [
            "横列の敵部隊を撃破する編成",
            "副将は姜燕や録嗚未もおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "虎刃双威伝",
              "effect": "習得した武将の策略+4 習得した武将の戦法に以下の効果を追加 ●攻撃対象の部隊の攻撃₋2%（効果時間6秒） ●攻撃対象の部隊の戦威₋2%（効果時間6秒）"
            },
            {
              "name": "攻防低下の激心",
              "effect": "習得した武将の戦法による攻撃低下の効果×1.05 習得した武将の戦法による防御低下の効果×1.05"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "防御低下の炎",
              "effect": "習得した武将の戦法による防御低下の効果時間+0秒 習得した武将の戦威+4"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            }
          ],
          "tipTitles": [
            "虎刃双威伝がおすすめ",
            "妨害目的なら攻防低下の激心もあり"
          ]
        }
      },
      "skills": [
        "掃討",
        "糧道",
        "飛槍",
        "俊才"
      ],
      "personalities": [
        "殲滅",
        "山陽の戦い",
        "廉頗軍",
        "忠誠心",
        "武人",
        "一騎討ち"
      ],
      "chainBase": 0.2,
      "attack": 808,
      "defense": 616,
      "war": 634,
      "strategy": 738
    },
    {
      "id": 4,
      "name": "騰",
      "rarity": "SSR",
      "tenpu": 900,
      "type": "援",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519319",
      "sourceId": "519319",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_4.png",
      "battleArtName": "廻剣妙技",
      "battleArtEffects": [
        "自部隊と同じ縦列の部隊の戦威を20％上昇させ、悠然を付与",
        "攻撃対象と同じ縦列の部隊に戦威200％の攻撃（攻撃対象には250％の攻撃）"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "早い",
      "guide": {
        "evaluationPoints": [
          "城砦の利権や調達で入手できる武将",
          "敵からのステータス低下を無効化"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "騰"
            },
            {
              "slot": "主将",
              "name": "王騎"
            },
            {
              "slot": "副将2",
              "name": "羌瘣"
            },
            {
              "slot": "補佐1",
              "name": "姜燕"
            },
            {
              "slot": "補佐2",
              "name": "楚水"
            }
          ],
          "selfSlot": "副将1",
          "focusTitles": [
            "戦法で敵部隊を撃破する編成",
            "軍勢の前列で使うのがおすすめ",
            "戦法で敵部隊を撃破する編成"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "廻刃裂陣伝",
              "effect": "習得した武将の戦法威力×1.03 習得した武将の戦法による戦威上昇の効果×1.04 編成した部隊に付与された戦威低下の効果時間-2秒"
            },
            {
              "name": "戦威上昇の激心",
              "effect": "習得した武将の戦法による戦威上昇の効果×1.06"
            },
            {
              "name": "城砦千伝・防戦",
              "effect": "編成した部隊の防御+1% 編成した部隊の戦威+1%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            },
            {
              "name": "城砦伍伝・戦策",
              "effect": "編成した部隊の戦威+0.2% 編成した部隊の策略+0.2%"
            }
          ],
          "tipTitles": [
            "廻刃裂陣伝がおすすめ",
            "戦威が上昇する秘伝と相性が良い"
          ]
        }
      },
      "skills": [
        "猛者",
        "統制",
        "腹心",
        "破壁"
      ],
      "personalities": [
        "支援",
        "馬陽の戦い",
        "王騎軍",
        "補佐役",
        "先陣をきる者",
        "戦術眼"
      ],
      "chainBase": 0.2,
      "attack": 790,
      "defense": 816,
      "war": 808,
      "strategy": 634
    },
    {
      "id": 3,
      "name": "麃公",
      "rarity": "SSR",
      "tenpu": 900,
      "type": "闘",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519320",
      "sourceId": "519320",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_3.png",
      "battleArtName": "活火激発",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊に会心発生上昇20％を付与",
        "攻撃対象と同じ縦列の部隊に戦威250％の攻撃（自部隊が種火状態の場合300％、炎状態の場合350％、大炎状態の場合400％に威力上昇）"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "遅い",
      "guide": {
        "evaluationPoints": [
          "軍団戦で活躍する闘タイプ武将",
          "調達隊でも活躍する"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "蒙驁"
            },
            {
              "slot": "主将",
              "name": "麃公"
            },
            {
              "slot": "副将2",
              "name": "縛虎申"
            },
            {
              "slot": "補佐1",
              "name": "介子坊"
            },
            {
              "slot": "補佐2",
              "name": "宮元"
            }
          ],
          "selfSlot": "主将",
          "focusTitles": [
            "戦法で大ダメージを与える編成",
            "副将は羌瘣や臨武君もおすすめ",
            "会心攻撃で敵を撃破する編成"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "激炎豪昇伝",
              "effect": "習得した武将の戦法威力×1.03 習得した武将の戦法による会心発生上昇の効果時間+2秒 習得した武将の戦威+4"
            },
            {
              "name": "城砦千伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "戦法技巧",
              "effect": "習得した武将の戦法威力×1.04"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            }
          ],
          "tipTitles": [
            "激炎豪昇伝がおすすめ",
            "戦威ダメージも伸ばせる"
          ]
        }
      },
      "skills": [
        "尖撃",
        "洞察",
        "炎将",
        "逆境"
      ],
      "personalities": [
        "殲滅",
        "蛇甘平原の戦い",
        "麃公軍",
        "経験豊富",
        "一騎討ち",
        "先陣をきる者"
      ],
      "chainBase": 0.2,
      "attack": 825,
      "defense": 816,
      "war": 790,
      "strategy": 634
    },
    {
      "id": 14,
      "name": "シュンメン",
      "rarity": "SSR",
      "tenpu": 850,
      "type": "援",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519309",
      "sourceId": "519309",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_14.png",
      "battleArtName": "鳥牙族の疾駆",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の戦威を20％上昇させ、会心威力上昇20％を付与",
        "自部隊に攻撃速度上昇20％を付与"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "早い",
      "guide": {
        "evaluationPoints": [
          "戦法で味方の火力を伸ばす武将",
          "会心編成の補佐として使える"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "シュンメン"
            },
            {
              "slot": "主将",
              "name": "摎"
            },
            {
              "slot": "副将2",
              "name": "嬴政"
            },
            {
              "slot": "補佐1",
              "name": "漂"
            },
            {
              "slot": "補佐2",
              "name": "紫夏"
            }
          ],
          "selfSlot": "副将1",
          "focusTitles": [
            "戦法で縦列の敵を撃破する編成",
            "前列で使うのがおすすめ",
            "戦法と会心で火力を出す編成"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦千伝・攻戦",
              "effect": "編成した部隊の攻撃+1% 編成した部隊の戦威+1%"
            },
            {
              "name": "城砦千伝・戦",
              "effect": "編成した部隊の戦威+1%"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            }
          ],
          "tipTitles": [
            "城砦千伝・攻戦がおすすめ",
            "烈火戦姫伝も相性が良い"
          ]
        }
      },
      "skills": [
        "掃討",
        "開智",
        "猛者",
        "攻陣"
      ],
      "personalities": [
        "防御",
        "王弟反乱",
        "山の民",
        "剣客",
        "お調子者",
        "武人"
      ],
      "chainBase": 0.2,
      "attack": 729,
      "defense": 572,
      "war": 597,
      "strategy": 464
    },
    {
      "id": 12,
      "name": "タジフ",
      "rarity": "SSR",
      "tenpu": 850,
      "type": "闘",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519311",
      "sourceId": "519311",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_12.png",
      "battleArtName": "山界の鉄槌",
      "battleArtEffects": [
        "自部隊の防御を30％上昇させ、堅固5％を付与",
        "攻撃対象に対物300％の攻撃"
      ],
      "battleArtType": "対物",
      "battleArtChainOrder": "遅い",
      "guide": {
        "evaluationPoints": [
          "物体に大ダメージを与える武将",
          "城砦や温泉で入手できる"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "同金"
            },
            {
              "slot": "主将",
              "name": "オルド"
            },
            {
              "slot": "副将2",
              "name": "タジフ"
            },
            {
              "slot": "補佐1",
              "name": "姜燕"
            },
            {
              "slot": "補佐2",
              "name": "シュンメン"
            }
          ],
          "selfSlot": "副将2",
          "focusTitles": [
            "建物に大ダメージを与える編成",
            "前列に配置して使うのがおすすめ",
            "戦法で建物の耐久を削る編成"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦百伝・物",
              "effect": "編成した部隊の対物+1%"
            },
            {
              "name": "戦法技巧",
              "effect": "習得した武将の戦法威力×1.04"
            },
            {
              "name": "攻撃の鍛錬",
              "effect": "習得した武将の攻撃+5"
            },
            {
              "name": "戦威の鍛錬",
              "effect": "習得した武将の戦威+5"
            }
          ],
          "tipTitles": [
            "城砦百伝・物がおすすめ",
            "攻撃と戦威の鍛錬もおすすめ"
          ]
        }
      },
      "skills": [
        "鋭鋒",
        "統制",
        "攻城",
        "威風"
      ],
      "personalities": [
        "突撃",
        "王弟反乱",
        "山の民",
        "武人",
        "怪力",
        "真面目"
      ],
      "chainBase": 0.2,
      "attack": 738,
      "defense": 622,
      "war": 613,
      "strategy": 323
    },
    {
      "id": 13,
      "name": "ランカイ",
      "rarity": "SSR",
      "tenpu": 850,
      "type": "妨",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519310",
      "sourceId": "519310",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_13.png",
      "battleArtName": "獣の暴",
      "battleArtEffects": [
        "攻撃対象と同じ横列の部隊の攻撃、防御を10％低下（自部隊の兵力が50％以上の場合は",
        "攻撃低下が30％に効果上昇）",
        "攻撃対象と同じ横列の部隊に戦威200％の攻撃"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "遅い",
      "guide": {
        "evaluationPoints": [
          "戦法で敵部隊を妨害する武将",
          "前列部隊の補佐として優秀"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "バジオウ"
            },
            {
              "slot": "主将",
              "name": "輪虎"
            },
            {
              "slot": "副将2",
              "name": "ランカイ"
            },
            {
              "slot": "補佐1",
              "name": "成恢"
            },
            {
              "slot": "補佐2",
              "name": "隆国"
            }
          ],
          "selfSlot": "副将2",
          "focusTitles": [
            "横列の敵部隊を撃破する編成",
            "前列で使うのがおすすめ",
            "戦法で敵部隊を撃破する編成"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "攻防低下の激心",
              "effect": "習得した武将の戦法による攻撃低下の効果×1.05 習得した武将の戦法による防御低下の効果×1.05"
            },
            {
              "name": "城砦千伝・戦",
              "effect": "編成した部隊の戦威+1%"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "防御低下の炎",
              "effect": "習得した武将の戦法による防御低下の効果時間+0秒 習得した武将の戦威+4"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            }
          ],
          "tipTitles": [
            "攻防低下の激心がおすすめ",
            "妨害目的なら防御低下の炎にする"
          ]
        }
      },
      "skills": [
        "攻城",
        "練武",
        "守壁",
        "鋭鋒"
      ],
      "personalities": [
        "殲滅",
        "王弟反乱",
        "成蟜一派",
        "怪力",
        "荒くれ者",
        "忠誠心"
      ],
      "chainBase": 0.2,
      "attack": 812,
      "defense": 754,
      "war": 605,
      "strategy": 74
    },
    {
      "id": 71,
      "name": "介子坊",
      "rarity": "SSR",
      "tenpu": 850,
      "type": "闘",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/533478",
      "sourceId": "533478",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_71.png",
      "battleArtName": "正攻不遮",
      "battleArtEffects": [
        "自部隊の攻撃を30％上昇",
        "攻撃対象と同じ横列の部隊に戦威200％の攻撃（攻撃対象には350％の攻撃）"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "攻撃的な戦法を持つ武将",
          "補佐としても優秀"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "バジオウ"
            },
            {
              "slot": "主将",
              "name": "輪虎"
            },
            {
              "slot": "副将2",
              "name": "介子坊"
            },
            {
              "slot": "補佐1",
              "name": "シュンメン"
            },
            {
              "slot": "補佐2",
              "name": "ムタ"
            }
          ],
          "selfSlot": "副将2",
          "focusTitles": [
            "戦法と通常攻撃で横列の敵を攻撃する編成",
            "輪虎は録嗚未でも代用できる"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦千伝・戦",
              "effect": "編成した部隊の戦威+1%"
            },
            {
              "name": "城砦千伝・攻戦",
              "effect": "編成した部隊の攻撃+1% 編成した部隊の戦威+1%"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            },
            {
              "name": "減戦短縮の印",
              "effect": "習得した武将の戦威+4"
            }
          ],
          "tipTitles": [
            "城砦千伝・戦がおすすめ",
            "補佐なら城砦伍伝にする"
          ]
        }
      },
      "skills": [
        "尖撃",
        "練武",
        "発揚",
        "攻城"
      ],
      "personalities": [
        "突撃",
        "山陽の戦い",
        "廉頗軍",
        "激情家",
        "武人",
        "怪力"
      ],
      "chainBase": 0.2,
      "attack": 729,
      "defense": 680,
      "war": 597,
      "strategy": 489
    },
    {
      "id": 8,
      "name": "信(童)",
      "rarity": "SSR",
      "tenpu": 850,
      "type": "闘",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519315",
      "sourceId": "519315",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_8.png",
      "battleArtName": "信念の飛矢",
      "battleArtEffects": [
        "自部隊の攻撃を30％上昇",
        "攻撃対象に戦威300％の攻撃（自部隊の兵力が50％以下の場合、350％に威力上昇）"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "遅い",
      "guide": {
        "evaluationPoints": [
          "火力に特化した闘タイプ武将",
          "序盤は主将として使える"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "蒙驁"
            },
            {
              "slot": "主将",
              "name": "麃公"
            },
            {
              "slot": "副将2",
              "name": "信(童)"
            },
            {
              "slot": "補佐1",
              "name": "介子坊"
            },
            {
              "slot": "補佐2",
              "name": "姜燕"
            }
          ],
          "selfSlot": "副将2",
          "focusTitles": [
            "軍団戦で火力を発揮する編成",
            "軍勢の前列で使うのがおすすめ",
            "通常攻撃と戦法で火力を出す編成"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦千伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            },
            {
              "name": "減攻短縮の印",
              "effect": "習得した武将の攻撃+4"
            }
          ],
          "tipTitles": [
            "城砦千伝・攻がおすすめ",
            "火力を伸ばす秘伝と相性が良い"
          ]
        }
      },
      "skills": [
        "逆境",
        "練武",
        "攻陣",
        "尖撃"
      ],
      "personalities": [
        "突撃",
        "蛇甘平原の戦い",
        "馬陽の戦い",
        "飛信隊",
        "大将軍への夢",
        "先陣をきる者"
      ],
      "chainBase": 0.2,
      "attack": 705,
      "defense": 646,
      "war": 671,
      "strategy": 356
    },
    {
      "id": 10,
      "name": "壁",
      "rarity": "SSR",
      "tenpu": 850,
      "type": "護",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519313",
      "sourceId": "519313",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_10.png",
      "battleArtName": "実直奮励",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の攻撃を15％、防御を25％上昇させ会心発生上昇20％を付与"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "戦法で味方を強化する武将",
          "他の武将と共通する個性は少なめ"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "尚鹿"
            },
            {
              "slot": "主将",
              "name": "壁"
            },
            {
              "slot": "副将2",
              "name": "干央"
            },
            {
              "slot": "補佐1",
              "name": "徐完"
            },
            {
              "slot": "補佐2",
              "name": "ムタ"
            }
          ],
          "selfSlot": "主将",
          "focusTitles": [
            "会心攻撃で火力を出す編成",
            "中列で使うのがおすすめ",
            "横列の味方を強化する編成"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦千伝・攻戦",
              "effect": "編成した部隊の攻撃+1% 編成した部隊の戦威+1%"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "会心上昇の炎",
              "effect": "習得した武将の戦法による会心発生上昇の効果時間+0秒 習得した武将の攻撃+4"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "減攻短縮の印",
              "effect": "習得した武将の攻撃+4"
            }
          ],
          "tipTitles": [
            "城砦千伝・攻戦がおすすめ",
            "会心上昇の炎もおすすめ"
          ]
        }
      },
      "skills": [
        "堅実",
        "人望",
        "不屈",
        "奮戦"
      ],
      "personalities": [
        "防御",
        "蛇甘平原の戦い",
        "大王一派",
        "補佐役",
        "真面目",
        "基本戦術"
      ],
      "chainBase": 0.2,
      "attack": 581,
      "defense": 680,
      "war": 605,
      "strategy": 547
    },
    {
      "id": 72,
      "name": "姜燕",
      "rarity": "SSR",
      "tenpu": 850,
      "type": "援",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/533479",
      "sourceId": "533479",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_72.png",
      "battleArtName": "飛閃の鏑矢",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の戦威、対物を40％上昇",
        "攻撃対象に戦威100％の攻撃（自部隊が後列にいる場合、200％に威力上昇）"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "早い",
      "guide": {
        "evaluationPoints": [
          "強力なバフを持つ武将",
          "軍勢の後列で使うのがおすすめ"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "姜燕"
            },
            {
              "slot": "主将",
              "name": "廉頗"
            },
            {
              "slot": "副将2",
              "name": "介子坊"
            },
            {
              "slot": "補佐1",
              "name": "蒙毅"
            },
            {
              "slot": "補佐2",
              "name": "タジフ"
            }
          ],
          "selfSlot": "副将1",
          "focusTitles": [
            "姜燕の戦法で与ダメージUP",
            "軍勢の後列で使うのがおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦千伝・戦",
              "effect": "編成した部隊の戦威+1%"
            },
            {
              "name": "城砦千伝・攻戦",
              "effect": "編成した部隊の攻撃+1% 編成した部隊の戦威+1%"
            },
            {
              "name": "対物上昇の激魂",
              "effect": "習得した武将の戦法による対物上昇の効果×1.02 習得した武将の戦法による対物上昇の効果時間+2秒"
            },
            {
              "name": "対物上昇の大魂",
              "effect": "習得した武将の戦法による対物上昇の効果×1.05"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            }
          ],
          "tipTitles": [
            "城砦千伝・戦がおすすめ",
            "対物上昇の秘伝も相性が良い"
          ]
        }
      },
      "skills": [
        "猛者",
        "洞察",
        "助勢",
        "扶助"
      ],
      "personalities": [
        "支援",
        "山陽の戦い",
        "廉頗軍",
        "弓の名手",
        "武人",
        "器用"
      ],
      "chainBase": 0.2,
      "attack": 622,
      "defense": 514,
      "war": 680,
      "strategy": 664
    },
    {
      "id": 66,
      "name": "宮元",
      "rarity": "SSR",
      "tenpu": 850,
      "type": "妨",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/527705",
      "sourceId": "527705",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_66.png",
      "battleArtName": "冷徹無比",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の負傷兵を最大兵力の20%回復",
        "攻撃対象と同じ横列の部隊の攻撃を50%低下",
        "攻撃対象に戦威200%の攻撃"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "早い",
      "guide": {
        "evaluationPoints": [
          "耐久力に優れる妨タイプ武将",
          "呉慶の副将として使える"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "羌瘣"
            },
            {
              "slot": "主将",
              "name": "呉慶"
            },
            {
              "slot": "副将2",
              "name": "宮元"
            },
            {
              "slot": "補佐1",
              "name": "渕"
            },
            {
              "slot": "補佐2",
              "name": "鱗坊"
            }
          ],
          "selfSlot": "副将2",
          "focusTitles": [
            "戦法で敵を妨害する高耐久の編成",
            "宮元は副将2に編成しよう"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦千伝・防",
              "effect": "編成した部隊の防御+1%"
            },
            {
              "name": "城砦千伝・戦",
              "effect": "編成した部隊の戦威+1%"
            },
            {
              "name": "城砦千伝・攻戦",
              "effect": "編成した部隊の攻撃+1% 編成した部隊の戦威+1%"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            }
          ],
          "tipTitles": [
            "城砦千伝・防がおすすめ",
            "攻撃や戦威上昇の秘伝もおすすめ"
          ]
        }
      },
      "skills": [
        "尖撃",
        "開智",
        "堅実",
        "鋭鋒"
      ],
      "personalities": [
        "術策",
        "蛇甘平原の戦い",
        "魏軍",
        "武人",
        "戦術眼",
        "謀略家"
      ],
      "chainBase": 0.2,
      "attack": 563,
      "defense": 522,
      "war": 655,
      "strategy": 671
    },
    {
      "id": 84,
      "name": "成恢",
      "rarity": "SSR",
      "tenpu": 850,
      "type": "妨",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/546516",
      "sourceId": "546516",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_84.png",
      "battleArtName": "苦悶に至る毒",
      "battleArtEffects": [
        "攻撃対象と同じ横列の部隊の防御、策略を15％低下させ、病毒10％を付与"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "病毒で敵を妨害する武将",
          "補佐としては非常に優秀"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "羌瘣"
            },
            {
              "slot": "主将",
              "name": "李牧"
            },
            {
              "slot": "副将2",
              "name": "宮元"
            },
            {
              "slot": "補佐1",
              "name": "隆国"
            },
            {
              "slot": "補佐2",
              "name": "成恢"
            }
          ],
          "selfSlot": "補佐2",
          "focusTitles": [
            "複数のデバフで敵を妨害する編成",
            "軍勢の前列で使うのがおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "減攻短縮の印",
              "effect": "習得した武将の攻撃+4"
            },
            {
              "name": "減策短縮の印",
              "effect": "習得した武将の策略+4"
            },
            {
              "name": "攻撃の鍛錬",
              "effect": "習得した武将の攻撃+5"
            },
            {
              "name": "策略の鍛錬",
              "effect": "習得した武将の策略+5"
            }
          ],
          "tipTitles": [
            "補佐で発動する秘伝がおすすめ",
            "副将なら城砦千伝・策にする"
          ]
        }
      },
      "skills": [
        "掃討",
        "開智",
        "黒弓",
        "助勢"
      ],
      "personalities": [
        "殲滅",
        "函谷関の戦い",
        "韓軍",
        "謀略家",
        "基本戦術",
        "器用"
      ],
      "chainBase": 0.2,
      "attack": 597,
      "defense": 456,
      "war": 505,
      "strategy": 671
    },
    {
      "id": 15,
      "name": "成蟜",
      "rarity": "SSR",
      "tenpu": 850,
      "type": "妨",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519308",
      "sourceId": "519308",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_15.png",
      "battleArtName": "王族の厳命",
      "battleArtEffects": [
        "自部隊の策略を30％上昇",
        "攻撃対象と同じ横列の部隊の防御、策略を20％、戦威を40％低下"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "複数のデバフで敵を妨害する武将",
          "兵糧への任命として使える"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "玄峰"
            },
            {
              "slot": "主将",
              "name": "成蟜"
            },
            {
              "slot": "副将2",
              "name": "ムタ"
            },
            {
              "slot": "補佐1",
              "name": "尚鹿"
            },
            {
              "slot": "補佐2",
              "name": "成恢"
            }
          ],
          "selfSlot": "主将",
          "focusTitles": [
            "デバフで敵を妨害する編成",
            "中列で使うのがおすすめ",
            "横列の味方を強化する編成"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "策略低下の激炎",
              "effect": "習得した武将の戦法による策略低下の効果時間+2秒 習得した武将の策略+5"
            },
            {
              "name": "策略低下の大魂",
              "effect": "習得した武将の戦法による策略低下の効果×1.05 習得した武将の戦法による策略低下の効果時間+0秒"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "策略の鍛錬",
              "effect": "習得した武将の策略+5"
            }
          ],
          "tipTitles": [
            "策略低下の激炎がおすすめ",
            "任命で使うなら策略の鍛錬にする"
          ]
        }
      },
      "skills": [
        "枢機",
        "糧道",
        "衆兵",
        "威風"
      ],
      "personalities": [
        "主導",
        "王弟反乱",
        "成蟜一派",
        "政権闘争",
        "王の一族",
        "謀略家"
      ],
      "chainBase": 0.2,
      "attack": 406,
      "defense": 381,
      "war": 522,
      "strategy": 754
    },
    {
      "id": 9,
      "name": "漂",
      "rarity": "SSR",
      "tenpu": 850,
      "type": "援",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519314",
      "sourceId": "519314",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_9.png",
      "battleArtName": "夢追う剣",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の弱化効果を1つ解除し戦威を10％上昇させ、攻撃速度上昇20％を付与"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "横列の味方を支援する武将",
          "序盤は信(童)の副将で使える"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "騰"
            },
            {
              "slot": "主将",
              "name": "王騎"
            },
            {
              "slot": "副将2",
              "name": "漂"
            },
            {
              "slot": "補佐1",
              "name": "姜燕"
            },
            {
              "slot": "補佐2",
              "name": "楚水"
            }
          ],
          "selfSlot": "副将2",
          "focusTitles": [
            "王騎が戦法で大ダメージを与える編成",
            "前列で使うのがおすすめ",
            "通常攻撃と戦法で火力を出す編成"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            },
            {
              "name": "城砦伍伝・戦策",
              "effect": "編成した部隊の戦威+0.2% 編成した部隊の策略+0.2%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "減戦短縮の印",
              "effect": "習得した武将の戦威+4"
            },
            {
              "name": "減攻短縮の印",
              "effect": "習得した武将の攻撃+4"
            }
          ],
          "tipTitles": [
            "城砦伍伝・戦がおすすめ",
            "発動できるなら戦威上昇の激心にする"
          ]
        }
      },
      "skills": [
        "逆境",
        "洞察",
        "守壁",
        "扶助"
      ],
      "personalities": [
        "主導",
        "王弟反乱",
        "大王一派",
        "大将軍への夢",
        "先陣をきる者",
        "戦術眼"
      ],
      "chainBase": 0.2,
      "attack": 605,
      "defense": 671,
      "war": 572,
      "strategy": 664
    },
    {
      "id": 64,
      "name": "縛虎申",
      "rarity": "SSR",
      "tenpu": 850,
      "type": "闘",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/524924",
      "sourceId": "524924",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_64.png",
      "battleArtName": "不退転",
      "battleArtEffects": [
        "自部隊の攻撃、対物を10%上昇させて攻撃速度上昇を10%付与",
        "攻撃対象と同じ縦列の部隊に戦威200%の攻撃（自部隊の兵力が30%以下の場合300%に威力上昇）"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "戦法で自部隊を強化する武将",
          "副将として幅広く使える"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "縛虎申"
            },
            {
              "slot": "主将",
              "name": "麃公"
            },
            {
              "slot": "副将2",
              "name": "信"
            },
            {
              "slot": "補佐1",
              "name": "漂"
            },
            {
              "slot": "補佐2",
              "name": "ランカイ"
            }
          ],
          "selfSlot": "副将1",
          "focusTitles": [
            "物体に大ダメージを与える編成",
            "兵力が少ない時に火力が伸びる"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "攻物上昇の激心",
              "effect": "習得した武将の戦法による攻撃上昇の効果×1.05 習得した武将の戦法による対物上昇の効果×1.05"
            },
            {
              "name": "城砦千伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦千伝・戦",
              "effect": "編成した部隊の戦威+1%"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦百伝・物",
              "effect": "編成した部隊の対物+1%"
            }
          ],
          "tipTitles": [
            "攻物上昇の激心がおすすめ",
            "城砦千伝でも代用できる"
          ]
        }
      },
      "skills": [
        "掃討",
        "練武",
        "逆境",
        "猛者"
      ],
      "personalities": [
        "突撃",
        "蛇甘平原の戦い",
        "麃公軍",
        "激情家",
        "先陣をきる者",
        "武人"
      ],
      "chainBase": 0.2,
      "attack": 713,
      "defense": 505,
      "war": 646,
      "strategy": 547
    },
    {
      "id": 85,
      "name": "臨武君",
      "rarity": "SSR",
      "tenpu": 850,
      "type": "闘",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/546517",
      "sourceId": "546517",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_85.png",
      "battleArtName": "強者狩り",
      "battleArtEffects": [
        "攻撃対象と同じ横列の部隊に戦威250％の攻撃",
        "（自部隊の兵力が50％以上の場合、350％に威力上昇）",
        "自部隊と同じ横列の部隊の戦威を20％上昇"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "早い",
      "guide": {
        "evaluationPoints": [
          "火力に特化した武将",
          "自身で戦法のバフは活かせない"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "臨武君"
            },
            {
              "slot": "主将",
              "name": "汗明"
            },
            {
              "slot": "副将2",
              "name": "介子坊"
            },
            {
              "slot": "補佐1",
              "name": "徐完"
            },
            {
              "slot": "補佐2",
              "name": "ムタ"
            }
          ],
          "selfSlot": "副将1",
          "focusTitles": [
            "敵軍勢に大ダメージを与える編成",
            "前列で使うのがおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "戦威上昇の激心",
              "effect": "習得した武将の戦法による戦威上昇の効果×1.06"
            },
            {
              "name": "城砦千伝・攻戦",
              "effect": "編成した部隊の攻撃+1% 編成した部隊の戦威+1%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            },
            {
              "name": "減戦短縮の印",
              "effect": "習得した武将の戦威+4"
            },
            {
              "name": "戦威の鍛錬",
              "effect": "習得した武将の戦威+5"
            }
          ],
          "tipTitles": [
            "戦威上昇の激心がおすすめ",
            "戦威強化の秘伝と相性が良い"
          ]
        }
      },
      "skills": [
        "鋭鋒",
        "人望",
        "攻陣",
        "頑健"
      ],
      "personalities": [
        "突撃",
        "函谷関の戦い",
        "楚軍",
        "真面目",
        "先陣をきる者",
        "一騎討ち"
      ],
      "chainBase": 0.2,
      "attack": 729,
      "defense": 622,
      "war": 605,
      "strategy": 480
    },
    {
      "id": 11,
      "name": "録嗚未",
      "rarity": "SSR",
      "tenpu": 850,
      "type": "闘",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519312",
      "sourceId": "519312",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_11.png",
      "battleArtName": "血気十全",
      "battleArtEffects": [
        "攻撃対象と同じ縦列の部隊に戦威250％の攻撃（自部隊が前列にいる場合、300％に威力上昇）"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "遅い",
      "guide": {
        "evaluationPoints": [
          "城砦や温泉で入手できる武将",
          "大勇のステータス上昇が優秀"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "羌瘣"
            },
            {
              "slot": "主将",
              "name": "信(臨時千人将)"
            },
            {
              "slot": "副将2",
              "name": "縛虎申"
            },
            {
              "slot": "補佐1",
              "name": "録嗚未"
            },
            {
              "slot": "補佐2",
              "name": "介子坊"
            }
          ],
          "selfSlot": "補佐1",
          "focusTitles": [
            "戦法で撃心ダメージを与える編成",
            "尖撃の効果で戦威ステータスUP",
            "高威力の戦法攻撃を行う編成"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦千伝・戦",
              "effect": "編成した部隊の戦威+1%"
            },
            {
              "name": "城砦千伝・攻戦",
              "effect": "編成した部隊の攻撃+1% 編成した部隊の戦威+1%"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            },
            {
              "name": "減戦短縮の印",
              "effect": "習得した武将の戦威+4"
            }
          ],
          "tipTitles": [
            "城砦千伝・戦がおすすめ",
            "補佐なら城砦伍伝・戦にする"
          ]
        }
      },
      "skills": [
        "尖撃",
        "練武",
        "大勇",
        "猛者"
      ],
      "personalities": [
        "突撃",
        "馬陽の戦い",
        "王騎軍",
        "忠誠心",
        "激情家",
        "武人"
      ],
      "chainBase": 0.2,
      "attack": 729,
      "defense": 721,
      "war": 630,
      "strategy": 480
    },
    {
      "id": 74,
      "name": "関常",
      "rarity": "SSR",
      "tenpu": 850,
      "type": "護",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/533487",
      "sourceId": "533487",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_74.png",
      "battleArtName": "当機立断",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の防御を40％上昇",
        "自部隊の戦威を40％上昇させ、堅固5％を付与"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "早い",
      "guide": {
        "evaluationPoints": [
          "耐久寄りの性能を持つ武将",
          "王賁の副将として使える"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "関常"
            },
            {
              "slot": "主将",
              "name": "王賁"
            },
            {
              "slot": "副将2",
              "name": "姜燕"
            },
            {
              "slot": "補佐1",
              "name": "昌文君"
            },
            {
              "slot": "補佐2",
              "name": "魏加"
            }
          ],
          "selfSlot": "副将1",
          "focusTitles": [
            "戦法で大ダメージを与える編成",
            "関常は副将1に編成しよう"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦千伝・戦",
              "effect": "編成した部隊の戦威+1%"
            },
            {
              "name": "城砦千伝・攻戦",
              "effect": "編成した部隊の攻撃+1% 編成した部隊の戦威+1%"
            },
            {
              "name": "城砦千伝・防戦",
              "effect": "編成した部隊の防御+1% 編成した部隊の戦威+1%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            },
            {
              "name": "減戦短縮の印",
              "effect": "習得した武将の戦威+4"
            }
          ],
          "tipTitles": [
            "城砦千伝・戦がおすすめ",
            "戦威上昇の秘伝と相性が良い"
          ]
        }
      },
      "skills": [
        "尖撃",
        "統制",
        "頑健",
        "俊才"
      ],
      "personalities": [
        "防御",
        "玉鳳隊",
        "補佐役",
        "真面目",
        "慎重",
        "武人"
      ],
      "chainBase": 0.2,
      "attack": 597,
      "defense": 680,
      "war": 555,
      "strategy": 597
    },
    {
      "id": 75,
      "name": "陸仙",
      "rarity": "SSR",
      "tenpu": 850,
      "type": "援",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/533488",
      "sourceId": "533488",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_75.png",
      "battleArtName": "扶翼の烈槍",
      "battleArtEffects": [
        "自部隊と同じ横列の攻撃を30％上昇させ、会心威力上昇30％を付与",
        "攻撃対象に戦威250％の攻撃"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "蒙恬の副将に最適な武将",
          "会心部隊の補佐でも使える"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "漂"
            },
            {
              "slot": "主将",
              "name": "蒙恬"
            },
            {
              "slot": "副将2",
              "name": "陸仙"
            },
            {
              "slot": "補佐1",
              "name": "嬴政"
            },
            {
              "slot": "補佐2",
              "name": "紫夏"
            }
          ],
          "selfSlot": "副将2",
          "focusTitles": [
            "会心攻撃でダメージを与える編成",
            "蒙恬は壁でも代用できる"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦千伝・防戦",
              "effect": "編成した部隊の防御+1% 編成した部隊の戦威+1%"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦百伝・防",
              "effect": "編成した部隊の防御+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "減攻短縮の印",
              "effect": "習得した武将の攻撃+4"
            }
          ],
          "tipTitles": [
            "城砦千伝・防戦がおすすめ",
            "攻撃上昇の秘伝もおすすめ"
          ]
        }
      },
      "skills": [
        "鋭鋒",
        "開智",
        "猛者",
        "発揚"
      ],
      "personalities": [
        "防御",
        "山陽の戦い",
        "楽華隊",
        "補佐役",
        "槍名人",
        "器用"
      ],
      "chainBase": 0.2,
      "attack": 671,
      "defense": 630,
      "war": 622,
      "strategy": 505
    },
    {
      "id": 87,
      "name": "項翼",
      "rarity": "SSR",
      "tenpu": 850,
      "type": "闘",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/546546",
      "sourceId": "546546",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_87.png",
      "battleArtName": "嚇なす妖刀",
      "battleArtEffects": [
        "自部隊の防御を25％上昇",
        "攻撃対象と同じ縦列の部隊に戦威250％の攻撃",
        "攻撃対象と同じ縦列の部隊の防御を30％低下"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "補佐として使える闘タイプ武将",
          "入手方法は課金のみ"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "羌瘣"
            },
            {
              "slot": "主将",
              "name": "信(臨時千人将)"
            },
            {
              "slot": "副将2",
              "name": "縛虎申"
            },
            {
              "slot": "補佐1",
              "name": "項翼"
            },
            {
              "slot": "補佐2",
              "name": "録嗚未"
            }
          ],
          "selfSlot": "補佐1",
          "focusTitles": [
            "戦法で撃心ダメージを与える編成",
            "項翼は録嗚未の副将でも使える"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "攻防の秘奥",
              "effect": "習得した武将の攻撃+10 習得した武将の防御+10"
            },
            {
              "name": "城砦千伝・戦",
              "effect": "編成した部隊の戦威+1%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            },
            {
              "name": "城砦伍伝・戦策",
              "effect": "編成した部隊の戦威+0.2% 編成した部隊の策略+0.2%"
            },
            {
              "name": "減戦短縮の印",
              "effect": "習得した武将の戦威+4"
            }
          ],
          "tipTitles": [
            "攻防の秘奥がおすすめ",
            "副将なら城砦千伝・戦にする"
          ]
        }
      },
      "skills": [
        "発揚",
        "練武",
        "大勇",
        "掃討"
      ],
      "personalities": [
        "突撃",
        "函谷関の戦い",
        "楚軍",
        "剣客",
        "荒くれ者",
        "激情家"
      ],
      "chainBase": 0.2,
      "attack": 721,
      "defense": 588,
      "war": 613,
      "strategy": 415
    },
    {
      "id": 88,
      "name": "オギコ",
      "rarity": "SSR",
      "tenpu": 800,
      "type": "妨",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/546547",
      "sourceId": "546547",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_88.png",
      "battleArtName": "無垢の遵従",
      "battleArtEffects": [
        "攻撃対象と同じ横列の部隊の策略を30％低下させ、それぞれ確率で攻撃、防御、戦威を30％低下"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "敵部隊の妨害に特化した武将",
          "入手方法は課金のみ"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "嬴政"
            },
            {
              "slot": "主将",
              "name": "摎"
            },
            {
              "slot": "副将2",
              "name": "羌瘣"
            },
            {
              "slot": "補佐1",
              "name": "オギコ"
            },
            {
              "slot": "補佐2",
              "name": "楚水"
            }
          ],
          "selfSlot": "補佐1",
          "focusTitles": [
            "戦法で敵部隊を撃破する編成",
            "シーズン3は雄心がLv3になる"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "攻防低下の大心",
              "effect": "習得した武将の戦法による攻撃低下の効果×1.04 習得した武将の戦法による防御低下の効果×1.04"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            },
            {
              "name": "減攻短縮の印",
              "effect": "習得した武将の攻撃+4"
            },
            {
              "name": "減戦短縮の印",
              "effect": "習得した武将の戦威+4"
            }
          ],
          "tipTitles": [
            "攻防低下の大心がおすすめ",
            "補佐なら城砦伍伝にする"
          ]
        }
      },
      "skills": [
        "助勢",
        "糧道",
        "奇矯",
        "雄心"
      ],
      "personalities": [
        "支援",
        "函谷関の戦い",
        "桓騎軍",
        "お調子者",
        "忠誠心",
        "剣客"
      ],
      "chainBase": 0.2,
      "attack": 591,
      "defense": 543,
      "war": 454,
      "strategy": 119
    },
    {
      "id": 86,
      "name": "バミュウ",
      "rarity": "SSR",
      "tenpu": 800,
      "type": "援",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/546518",
      "sourceId": "546518",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_86.png",
      "battleArtName": "喜悦の従属",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の攻撃、策略を20％上昇",
        "（自身が副将の場合、40％に効果上昇）"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "早い",
      "guide": {
        "evaluationPoints": [
          "副将で性能を発揮する武将",
          "共通する個性を持つ武将が少ない"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "宮元"
            },
            {
              "slot": "主将",
              "name": "王翦"
            },
            {
              "slot": "副将2",
              "name": "漂"
            },
            {
              "slot": "補佐1",
              "name": "干央"
            },
            {
              "slot": "補佐2",
              "name": "バミュウ"
            }
          ],
          "selfSlot": "補佐2",
          "focusTitles": [
            "反撃で敵部隊を撃破する編成",
            "前列で使うのがおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "城砦伍伝・防",
              "effect": "編成した部隊の防御+0.4%"
            },
            {
              "name": "攻撃の鍛錬",
              "effect": "習得した武将の攻撃+5"
            },
            {
              "name": "防御の鍛錬",
              "effect": "習得した武将の防御+5"
            }
          ],
          "tipTitles": [
            "補佐で発動する秘伝がおすすめ",
            "副将なら城砦千伝・攻防にする"
          ]
        }
      },
      "skills": [
        "扶助",
        "糧道",
        "鋭鋒",
        "守壁"
      ],
      "personalities": [
        "支援",
        "函谷関の戦い",
        "楚軍",
        "お調子者",
        "補佐役",
        "忠誠心"
      ],
      "chainBase": 0.2,
      "attack": 583,
      "defense": 566,
      "war": 527,
      "strategy": 640
    },
    {
      "id": 63,
      "name": "ムタ",
      "rarity": "SSR",
      "tenpu": 800,
      "type": "妨",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/522277",
      "sourceId": "522277",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_63.png",
      "battleArtName": "脅威の毒矢",
      "battleArtEffects": [
        "攻撃対象と同じ横列の部隊の攻撃を15%低下させ、病毒5%を付与"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "病毒を付与するデバフ戦法",
          "成蟜一派と相性がいい"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "シュンメン"
            },
            {
              "slot": "主将",
              "name": "楊端和"
            },
            {
              "slot": "副将2",
              "name": "バジオウ"
            },
            {
              "slot": "補佐1",
              "name": "ムタ"
            },
            {
              "slot": "補佐2",
              "name": "徐完"
            }
          ],
          "selfSlot": "補佐1",
          "focusTitles": [
            "鋭鋒と掃討で部隊の火力UP",
            "成蟜の副将でも使える"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "減攻短縮の印",
              "effect": "習得した武将の攻撃+4"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            },
            {
              "name": "減戦短縮の印",
              "effect": "習得した武将の戦威+4"
            },
            {
              "name": "策略の鍛錬",
              "effect": "習得した武将の策略+5"
            }
          ],
          "tipTitles": [
            "補佐で発動する秘伝がおすすめ",
            "任命で使うなら策略の鍛錬にする"
          ]
        }
      },
      "skills": [
        "鋭鋒",
        "練武",
        "掃討",
        "大勇"
      ],
      "personalities": [
        "術策",
        "王弟反乱",
        "成蟜一派",
        "暗躍",
        "お調子者"
      ],
      "chainBase": 0.2,
      "attack": 623,
      "defense": 566,
      "war": 607,
      "strategy": 607
    },
    {
      "id": 24,
      "name": "同金",
      "rarity": "SSR",
      "tenpu": 800,
      "type": "援",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519299",
      "sourceId": "519299",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_24.png",
      "battleArtName": "寡言豪胆",
      "battleArtEffects": [
        "自部隊同じ横列の部隊の攻撃、対物を30％上昇"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "早い",
      "guide": {
        "evaluationPoints": [
          "戦法のバフが優秀な武将",
          "城砦や温泉で入手できる"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "同金"
            },
            {
              "slot": "主将",
              "name": "オルド"
            },
            {
              "slot": "副将2",
              "name": "タジフ"
            },
            {
              "slot": "補佐1",
              "name": "ランカイ"
            },
            {
              "slot": "補佐2",
              "name": "シュンメン"
            }
          ],
          "selfSlot": "副将1",
          "focusTitles": [
            "建物の耐久を削る編成",
            "前列で使うのがおすすめ",
            "横列の味方を強化する編成"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "対物上昇の激魂",
              "effect": "習得した武将の戦法による対物上昇の効果×1.02 習得した武将の戦法による対物上昇の効果時間+2秒"
            },
            {
              "name": "攻物上昇の激心",
              "effect": "習得した武将の戦法による攻撃上昇の効果×1.05 習得した武将の戦法による対物上昇の効果×1.05"
            },
            {
              "name": "対物上昇の大魂",
              "effect": "習得した武将の戦法による対物上昇の効果×1.05"
            },
            {
              "name": "城砦百伝・物",
              "effect": "編成した部隊の対物+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            }
          ],
          "tipTitles": [
            "対物上昇の激魂がおすすめ",
            "対物系の秘伝と相性が良い"
          ]
        }
      },
      "skills": [
        "鋭鋒",
        "統制",
        "破壁",
        "扶助"
      ],
      "personalities": [
        "支援",
        "馬陽の戦い",
        "王騎軍",
        "怪力",
        "武人"
      ],
      "chainBase": 0.2,
      "attack": 671,
      "defense": 663,
      "war": 599,
      "strategy": 447
    },
    {
      "id": 30,
      "name": "向",
      "rarity": "SSR",
      "tenpu": 800,
      "type": "援",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519293",
      "sourceId": "519293",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_30.png",
      "battleArtName": "心照らす灯火",
      "battleArtEffects": [
        "自部隊と同じ縦列の部隊の攻撃を30％上昇させ、弱化効果を1つ解除"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "任命で活躍する武将",
          "救援効果で負傷兵を回復"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "尚鹿"
            },
            {
              "slot": "主将",
              "name": "壁"
            },
            {
              "slot": "副将2",
              "name": "渕"
            },
            {
              "slot": "補佐1",
              "name": "向"
            },
            {
              "slot": "補佐2",
              "name": "鱗坊"
            }
          ],
          "selfSlot": "補佐1",
          "focusTitles": [
            "反撃で敵にダメージを与える編成",
            "嬴政や楚水の副将もおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "城砦伍伝・防",
              "effect": "編成した部隊の防御+0.4%"
            },
            {
              "name": "減攻短縮の印",
              "effect": "習得した武将の攻撃+4"
            },
            {
              "name": "減防短縮の印",
              "effect": "習得した武将の防御+4"
            },
            {
              "name": "策略の鍛錬",
              "effect": "習得した武将の策略+5"
            }
          ],
          "tipTitles": [
            "補佐で発動する秘伝がおすすめ",
            "任命で使うなら策略の鍛錬にする"
          ]
        }
      },
      "skills": [
        "堅実",
        "糧道",
        "献身",
        "扶助"
      ],
      "personalities": [
        "支援",
        "大王一派",
        "後宮",
        "平穏を祈る者",
        "慎重",
        "忠誠心"
      ],
      "chainBase": 0.2,
      "attack": 351,
      "defense": 463,
      "war": 359,
      "strategy": 495
    },
    {
      "id": 26,
      "name": "尚鹿",
      "rarity": "SSR",
      "tenpu": 800,
      "type": "護",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519297",
      "sourceId": "519297",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_26.png",
      "battleArtName": "騎馬飄々",
      "battleArtEffects": [
        "自部隊と同じ縦列の部隊の防御を20％上昇させ、会心威力上昇10％を付与（自部隊が中列にいる場合",
        "会心威力上昇が20％に効果上昇）"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "会心編成の副将で使える武将",
          "調達隊でも活躍する"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "尚鹿"
            },
            {
              "slot": "主将",
              "name": "麃公"
            },
            {
              "slot": "副将2",
              "name": "縛虎申"
            },
            {
              "slot": "補佐1",
              "name": "ムタ"
            },
            {
              "slot": "補佐2",
              "name": "シュンメン"
            }
          ],
          "selfSlot": "副将1",
          "focusTitles": [
            "高威力の会心ダメージを与える編成",
            "中列で使うのがおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "減攻短縮の印",
              "effect": "習得した武将の攻撃+4"
            },
            {
              "name": "攻撃の鍛錬",
              "effect": "習得した武将の攻撃+5"
            },
            {
              "name": "攻防の鍛錬",
              "effect": "習得した武将の攻撃+3 習得した武将の防御+3"
            }
          ],
          "tipTitles": [
            "城砦百伝・攻がおすすめ",
            "補佐なら城砦伍伝・攻にする"
          ]
        }
      },
      "skills": [
        "頑健",
        "洞察",
        "枢機",
        "奮戦"
      ],
      "personalities": [
        "防御",
        "蛇甘平原の戦い",
        "麃公軍",
        "補佐役",
        "慎重",
        "お調子者"
      ],
      "chainBase": 0.2,
      "attack": 591,
      "defense": 566,
      "war": 575,
      "strategy": 486
    },
    {
      "id": 18,
      "name": "干央",
      "rarity": "SSR",
      "tenpu": 800,
      "type": "闘",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519305",
      "sourceId": "519305",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_18.png",
      "battleArtName": "死闘敢戦",
      "battleArtEffects": [
        "攻撃対象と同じ横列の部隊に戦威150％の攻撃（自部隊の兵力が30％以下の場合、300％に威力上昇）"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "遅い",
      "guide": {
        "evaluationPoints": [
          "攻撃性能に特化した武将",
          "最序盤は主将として使える"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "騰"
            },
            {
              "slot": "主将",
              "name": "王騎"
            },
            {
              "slot": "副将2",
              "name": "干央"
            },
            {
              "slot": "補佐1",
              "name": "姜燕"
            },
            {
              "slot": "補佐2",
              "name": "楚水"
            }
          ],
          "selfSlot": "副将2",
          "focusTitles": [
            "戦法で敵部隊を撃破する編成",
            "副将は羌瘣や漂もおすすめ",
            "戦法で敵部隊を妨害する編成"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦千伝・戦",
              "effect": "編成した部隊の戦威+1%"
            },
            {
              "name": "城砦千伝・攻戦",
              "effect": "編成した部隊の攻撃+1% 編成した部隊の戦威+1%"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "減攻短縮の印",
              "effect": "習得した武将の攻撃+4"
            }
          ],
          "tipTitles": [
            "城砦千伝・戦がおすすめ",
            "攻撃上昇の秘伝とも相性が良い"
          ]
        }
      },
      "skills": [
        "鋭鋒",
        "統制",
        "奮戦",
        "不屈"
      ],
      "personalities": [
        "突撃",
        "馬陽の戦い",
        "王騎軍",
        "真面目",
        "武人"
      ],
      "chainBase": 0.2,
      "attack": 679,
      "defense": 694,
      "war": 566,
      "strategy": 463
    },
    {
      "id": 29,
      "name": "徐完",
      "rarity": "SSR",
      "tenpu": 800,
      "type": "闘",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519294",
      "sourceId": "519294",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_29.png",
      "battleArtName": "朱凶の剣",
      "battleArtEffects": [
        "攻撃対象と同じ縦列の部隊に戦威150％の攻撃（攻撃する敵部隊の兵力が30％以下の場合200％に威力上昇）"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "遅い",
      "guide": {
        "evaluationPoints": [
          "戦法で縦列を攻撃する武将",
          "会心編成の補佐として優秀"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "バジオウ"
            },
            {
              "slot": "主将",
              "name": "楊端和"
            },
            {
              "slot": "副将2",
              "name": "徐完"
            },
            {
              "slot": "補佐1",
              "name": "干央"
            },
            {
              "slot": "補佐2",
              "name": "楚水"
            }
          ],
          "selfSlot": "副将2",
          "focusTitles": [
            "会心と戦法で敵にダメージを与える編成",
            "軍勢内の好きな位置で使える"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            },
            {
              "name": "減攻短縮の印",
              "effect": "習得した武将の攻撃+4"
            },
            {
              "name": "減戦短縮の印",
              "effect": "習得した武将の戦威+4"
            }
          ],
          "tipTitles": [
            "城砦百伝・攻がおすすめ",
            "補佐で発動する秘伝もおすすめ"
          ]
        }
      },
      "skills": [
        "鋭鋒",
        "練武",
        "豪傑",
        "奮戦"
      ],
      "personalities": [
        "殲滅",
        "王弟反乱",
        "成蟜一派",
        "暗躍",
        "剣客"
      ],
      "chainBase": 0.2,
      "attack": 623,
      "defense": 607,
      "war": 519,
      "strategy": 454
    },
    {
      "id": 22,
      "name": "昌文君",
      "rarity": "SSR",
      "tenpu": 800,
      "type": "護",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519301",
      "sourceId": "519301",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_22.png",
      "battleArtName": "王の柱石",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の防御、策略を25％上昇",
        "攻撃対象に戦威150％の攻撃"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "早い",
      "guide": {
        "evaluationPoints": [
          "調達隊でも活躍する"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "昌文君"
            },
            {
              "slot": "主将",
              "name": "嬴政"
            },
            {
              "slot": "副将2",
              "name": "宮元"
            },
            {
              "slot": "補佐1",
              "name": "壁"
            },
            {
              "slot": "補佐2",
              "name": "紫夏"
            }
          ],
          "selfSlot": "副将1",
          "focusTitles": [
            "嬴政の戦法で味方を強化する編成",
            "後列で使うのがおすすめ",
            "恐怖で敵部隊を妨害する編成"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦千伝・防戦",
              "effect": "編成した部隊の防御+1% 編成した部隊の戦威+1%"
            },
            {
              "name": "城砦百伝・防",
              "effect": "編成した部隊の防御+1%"
            },
            {
              "name": "城砦伍伝・防",
              "effect": "編成した部隊の防御+0.4%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            },
            {
              "name": "減戦短縮の印",
              "effect": "習得した武将の戦威+4"
            }
          ],
          "tipTitles": [
            "城砦千伝・防戦がおすすめ",
            "防御か戦威上昇の秘伝と相性が良い"
          ]
        }
      },
      "skills": [
        "尖撃",
        "洞察",
        "防陣",
        "堅実"
      ],
      "personalities": [
        "支援",
        "王弟反乱",
        "大王一派",
        "補佐役",
        "中華統一",
        "経験豊富"
      ],
      "chainBase": 0.2,
      "attack": 591,
      "defense": 607,
      "war": 623,
      "strategy": 646
    },
    {
      "id": 21,
      "name": "楚水",
      "rarity": "SSR",
      "tenpu": 800,
      "type": "援",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519302",
      "sourceId": "519302",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_21.png",
      "battleArtName": "忠武突貫",
      "battleArtEffects": [
        "自部隊の攻撃を20％上昇（自部隊が前列にいる場合、30％に効果上昇）",
        "攻撃対象と同じ縦列の部隊に戦威100％の攻撃"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "遅い",
      "guide": {
        "evaluationPoints": [
          "攻撃的な戦法を持つ武将",
          "序盤は主将として使える"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "羌瘣"
            },
            {
              "slot": "主将",
              "name": "信(臨時千人将)"
            },
            {
              "slot": "副将2",
              "name": "楚水"
            },
            {
              "slot": "補佐1",
              "name": "録嗚未"
            },
            {
              "slot": "補佐2",
              "name": "姜燕"
            }
          ],
          "selfSlot": "副将2",
          "focusTitles": [
            "戦法で撃心ダメージを与える編成",
            "前列で使うのがおすすめ",
            "シーズン1の序盤用編成"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦千伝・戦",
              "effect": "編成した部隊の戦威+1%"
            },
            {
              "name": "城砦千伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            }
          ],
          "tipTitles": [
            "城砦千伝・戦がおすすめ",
            "補佐なら城砦伍伝にする"
          ]
        }
      },
      "skills": [
        "掃討",
        "統制",
        "助勢",
        "奮戦"
      ],
      "personalities": [
        "支援",
        "山陽の戦い",
        "飛信隊",
        "補佐役",
        "器用",
        "忠誠心"
      ],
      "chainBase": 0.2,
      "attack": 575,
      "defense": 583,
      "war": 646,
      "strategy": 543
    },
    {
      "id": 16,
      "name": "河了貂",
      "rarity": "SSR",
      "tenpu": 800,
      "type": "護",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519307",
      "sourceId": "519307",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_16.png",
      "battleArtName": "河了貂也",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の戦威、策略を15％上昇させ負傷兵を最大兵力の20％回復"
      ],
      "battleArtType": "回復",
      "battleArtChainOrder": "早い",
      "guide": {
        "evaluationPoints": [
          "後列の補佐として使える武将",
          "序盤は楚水の副将として使える"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "河了貂"
            },
            {
              "slot": "主将",
              "name": "王翦"
            },
            {
              "slot": "副将2",
              "name": "宮元"
            },
            {
              "slot": "補佐1",
              "name": "魏加"
            },
            {
              "slot": "補佐2",
              "name": "黄離弦"
            }
          ],
          "selfSlot": "副将1",
          "focusTitles": [
            "耐久力に優れた対部隊編成",
            "後列で使うのがおすすめ",
            "恐怖で敵部隊を妨害する編成"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦千伝・防戦",
              "effect": "編成した部隊の防御+1% 編成した部隊の戦威+1%"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦百伝・防",
              "effect": "編成した部隊の防御+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "減攻短縮の印",
              "effect": "習得した武将の攻撃+4"
            }
          ],
          "tipTitles": [
            "城砦千伝・防戦がおすすめ",
            "補佐なら城砦伍伝・攻にする"
          ]
        }
      },
      "skills": [
        "聡明",
        "糧道",
        "後備",
        "扶助"
      ],
      "personalities": [
        "支援",
        "王弟反乱",
        "補佐役",
        "器用",
        "お調子者",
        "戦術眼"
      ],
      "chainBase": 0.2,
      "attack": 447,
      "defense": 495,
      "war": 560,
      "strategy": 640
    },
    {
      "id": 17,
      "name": "渕",
      "rarity": "SSR",
      "tenpu": 800,
      "type": "護",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519306",
      "sourceId": "519306",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_17.png",
      "battleArtName": "集の全力",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の防御40％上昇"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "耐久性能に特化した武将",
          "壁や白亀西の副将で使える"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "渕"
            },
            {
              "slot": "主将",
              "name": "壁"
            },
            {
              "slot": "副将2",
              "name": "陸仙"
            },
            {
              "slot": "補佐1",
              "name": "シュンメン"
            },
            {
              "slot": "補佐2",
              "name": "徐完"
            }
          ],
          "selfSlot": "副将1",
          "focusTitles": [
            "反撃と会心で敵を撃破する編成",
            "シーズン3は前列で使おう",
            "戦法で味方を強化する編成"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦千伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦千伝・防",
              "effect": "編成した部隊の防御+1%"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "護兵増員・中",
              "effect": "編成した部隊の兵力+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            }
          ],
          "tipTitles": [
            "城砦千伝・攻がおすすめ",
            "秘伝で攻撃か防御を強化しよう"
          ]
        }
      },
      "skills": [
        "頑健",
        "糧道",
        "堅実",
        "守壁"
      ],
      "personalities": [
        "防御",
        "馬陽の戦い",
        "飛信隊",
        "補佐役",
        "真面目"
      ],
      "chainBase": 0.2,
      "attack": 551,
      "defense": 640,
      "war": 566,
      "strategy": 511
    },
    {
      "id": 73,
      "name": "玄峰",
      "rarity": "SSR",
      "tenpu": 800,
      "type": "妨",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/533480",
      "sourceId": "533480",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_73.png",
      "battleArtName": "盤上遊戯",
      "battleArtEffects": [
        "自部隊に確率回避30％を付与",
        "攻撃対象と同じ横列の部隊の防御、策略を35％低下"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "対軍勢の時に活躍する武将",
          "補佐で使うのがおすすめ"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "バジオウ"
            },
            {
              "slot": "主将",
              "name": "輪虎"
            },
            {
              "slot": "副将2",
              "name": "介子坊"
            },
            {
              "slot": "補佐1",
              "name": "黄離弦"
            },
            {
              "slot": "補佐2",
              "name": "玄峰"
            }
          ],
          "selfSlot": "補佐2",
          "focusTitles": [
            "デバフで敵を弱体化させる編成",
            "後列で使うのがおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "城砦伍伝・策",
              "effect": "編成した部隊の策略+0.4%"
            },
            {
              "name": "減攻短縮の印",
              "effect": "習得した武将の攻撃+4"
            },
            {
              "name": "減策短縮の印",
              "effect": "習得した武将の策略+4"
            },
            {
              "name": "策略の鍛錬",
              "effect": "習得した武将の策略+5"
            }
          ],
          "tipTitles": [
            "補佐で発動する秘伝がおすすめ",
            "任命で使うなら策略の鍛錬にする"
          ]
        }
      },
      "skills": [
        "発揚",
        "糧道",
        "献策",
        "衆兵"
      ],
      "personalities": [
        "殲滅",
        "山陽の戦い",
        "廉頗軍",
        "謀略家",
        "教育係",
        "経験豊富"
      ],
      "chainBase": 0.2,
      "attack": 447,
      "defense": 560,
      "war": 655,
      "strategy": 687
    },
    {
      "id": 78,
      "name": "瑠衣",
      "rarity": "SSR",
      "tenpu": 800,
      "type": "援",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/540104",
      "sourceId": "540104",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_78.png",
      "battleArtName": "比翼連理",
      "battleArtEffects": [
        "自部隊と同じ縦列の攻撃、策略を30％上昇"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "成蟜の副将で使える援タイプ武将",
          "調達隊であれば特に活躍できる"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "蒙驁"
            },
            {
              "slot": "主将",
              "name": "麃公"
            },
            {
              "slot": "副将2",
              "name": "縛虎申"
            },
            {
              "slot": "補佐1",
              "name": "瑠衣"
            },
            {
              "slot": "補佐2",
              "name": "宮元"
            }
          ],
          "selfSlot": "補佐1",
          "focusTitles": [
            "負傷兵を回復しながら戦う編成",
            "成蟜や呉慶の副将もおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "攻策上昇の大心",
              "effect": "習得した武将の戦法による攻撃上昇の効果×1.04 習得した武将の戦法による策略上昇の効果×1.04"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "減攻短縮の印",
              "effect": "習得した武将の攻撃+4"
            },
            {
              "name": "攻撃の鍛錬",
              "effect": "習得した武将の攻撃+5"
            },
            {
              "name": "攻防の鍛錬",
              "effect": "習得した武将の攻撃+3 習得した武将の防御+3"
            }
          ],
          "tipTitles": [
            "攻策上昇の大心がおすすめ",
            "補佐で発動する秘伝もおすすめ"
          ]
        }
      },
      "skills": [
        "堅実",
        "洞察",
        "威風",
        "献身"
      ],
      "personalities": [
        "支援",
        "成蟜一派",
        "王の一族",
        "平穏を祈る者",
        "政権闘争",
        "忠誠心"
      ],
      "chainBase": 0.2,
      "attack": 374,
      "defense": 391,
      "war": 391,
      "strategy": 591
    },
    {
      "id": 25,
      "name": "白亀西",
      "rarity": "SSR",
      "tenpu": 800,
      "type": "援",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519298",
      "sourceId": "519298",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_25.png",
      "battleArtName": "軍威高揚",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の攻撃を40％上昇"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "戦法で味方を強化する武将",
          "王騎と組み合わせて使える"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "壁"
            },
            {
              "slot": "主将",
              "name": "白亀西"
            },
            {
              "slot": "副将2",
              "name": "縛虎申"
            },
            {
              "slot": "補佐1",
              "name": "干央"
            },
            {
              "slot": "補佐2",
              "name": "ムタ"
            }
          ],
          "selfSlot": "主将",
          "focusTitles": [
            "味方に強力なバフを付与する編成",
            "どの列に配置しても活躍する"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦千伝・攻防",
              "effect": "編成した部隊の攻撃+1% 編成した部隊の防御+1%"
            },
            {
              "name": "城砦千伝・攻戦",
              "effect": "編成した部隊の攻撃+1% 編成した部隊の戦威+1%"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            }
          ],
          "tipTitles": [
            "城砦千伝・攻防がおすすめ",
            "攻撃上昇の秘伝と相性が良い"
          ]
        }
      },
      "skills": [
        "威風",
        "人望",
        "衆兵",
        "防陣"
      ],
      "personalities": [
        "主導",
        "蛇甘平原の戦い",
        "魏軍",
        "激情家",
        "真面目",
        "補佐役"
      ],
      "chainBase": 0.2,
      "attack": 431,
      "defense": 439,
      "war": 495,
      "strategy": 431
    },
    {
      "id": 32,
      "name": "紫夏",
      "rarity": "SSR",
      "tenpu": 800,
      "type": "妨",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519291",
      "sourceId": "519291",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_32.png",
      "battleArtName": "月下の闇商",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の弱化効果を1つ解除",
        "攻撃対象と同じ横列の部隊の攻撃、策略を30％低下"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "早い",
      "guide": {
        "evaluationPoints": [
          "進物で入手できる妨タイプ武将",
          "摎や蒙恬と相性が良い"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "紫夏"
            },
            {
              "slot": "主将",
              "name": "蒙恬"
            },
            {
              "slot": "副将2",
              "name": "羌瘣"
            },
            {
              "slot": "補佐1",
              "name": "嬴政"
            },
            {
              "slot": "補佐2",
              "name": "陸仙"
            }
          ],
          "selfSlot": "副将1",
          "focusTitles": [
            "通常攻撃と反撃で敵を撃破する編成",
            "摎や蒙恬の補佐もおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "減攻短縮の印",
              "effect": "習得した武将の攻撃+4"
            },
            {
              "name": "攻撃の鍛錬",
              "effect": "習得した武将の攻撃+5"
            },
            {
              "name": "攻防の鍛錬",
              "effect": "習得した武将の攻撃+3 習得した武将の防御+3"
            }
          ],
          "tipTitles": [
            "城砦百伝・攻がおすすめ",
            "補佐なら減攻短縮の印にする"
          ]
        }
      },
      "skills": [
        "不屈",
        "人望",
        "雄心",
        "献策"
      ],
      "personalities": [
        "主導",
        "大王一派",
        "弓の名手",
        "器用",
        "暗躍",
        "戦術眼"
      ],
      "chainBase": 0.2,
      "attack": 439,
      "defense": 431,
      "war": 471,
      "strategy": 720
    },
    {
      "id": 27,
      "name": "蒙毅",
      "rarity": "SSR",
      "tenpu": 800,
      "type": "援",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519296",
      "sourceId": "519296",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_27.png",
      "battleArtName": "慧眼明察",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の策略、対物を30％上昇"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "戦法と技能で対物を強化する武将",
          "救援武将としても優秀"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "漂"
            },
            {
              "slot": "主将",
              "name": "王騎"
            },
            {
              "slot": "副将2",
              "name": "蒙毅"
            },
            {
              "slot": "補佐1",
              "name": "信"
            },
            {
              "slot": "補佐2",
              "name": "ランカイ"
            }
          ],
          "selfSlot": "副将2",
          "focusTitles": [
            "味方に複数のバフを付与する編成",
            "軍勢の前列で使うのがおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦千伝・防戦",
              "effect": "編成した部隊の防御+1% 編成した部隊の戦威+1%"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦百伝・物",
              "effect": "編成した部隊の対物+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            }
          ],
          "tipTitles": [
            "城砦千伝・防戦がおすすめ",
            "城砦百伝・物も相性が良い"
          ]
        }
      },
      "skills": [
        "攻城",
        "開智",
        "防陣",
        "威風"
      ],
      "personalities": [
        "術策",
        "馬陽の戦い",
        "戦術眼",
        "教育係",
        "補佐役"
      ],
      "chainBase": 0.2,
      "attack": 374,
      "defense": 623,
      "war": 591,
      "strategy": 646
    },
    {
      "id": 31,
      "name": "陽",
      "rarity": "SSR",
      "tenpu": 800,
      "type": "護",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519292",
      "sourceId": "519292",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_31.png",
      "battleArtName": "皆を包む陽光",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の防御を20%上昇させ負傷兵を最大兵力の20%回復"
      ],
      "battleArtType": "回復",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "負傷兵を回復する護タイプ武将",
          "兵糧への任命で使える"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "昌文君"
            },
            {
              "slot": "主将",
              "name": "楊端和"
            },
            {
              "slot": "副将2",
              "name": "バジオウ"
            },
            {
              "slot": "補佐1",
              "name": "陽"
            },
            {
              "slot": "補佐2",
              "name": "河了貂"
            }
          ],
          "selfSlot": "補佐1",
          "focusTitles": [
            "恐怖で敵を妨害する編成",
            "昌文君や楚水の副将もおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "城砦伍伝・防",
              "effect": "編成した部隊の防御+0.4%"
            },
            {
              "name": "減攻短縮の印",
              "effect": "習得した武将の攻撃+4"
            },
            {
              "name": "減防短縮の印",
              "effect": "習得した武将の防御+4"
            },
            {
              "name": "策略の鍛錬",
              "effect": "習得した武将の策略+5"
            }
          ],
          "tipTitles": [
            "補佐で発動する秘伝がおすすめ",
            "任命で使うなら策略の鍛錬にする"
          ]
        }
      },
      "skills": [
        "聡明",
        "糧道",
        "扶助",
        "枢機"
      ],
      "personalities": [
        "支援",
        "大王一派",
        "後宮",
        "平穏を祈る者",
        "お調子者",
        "器用"
      ],
      "chainBase": 0.2,
      "attack": 367,
      "defense": 400,
      "war": 374,
      "strategy": 511
    },
    {
      "id": 23,
      "name": "隆国",
      "rarity": "SSR",
      "tenpu": 800,
      "type": "援",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519300",
      "sourceId": "519300",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_23.png",
      "battleArtName": "知勇兼備",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の策略を40％上昇",
        "攻撃対象と同じ横列の部隊に戦威150％の攻撃"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "策略ステータスを上昇させる武将",
          "城砦や温泉で入手できる"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "騰"
            },
            {
              "slot": "主将",
              "name": "王騎"
            },
            {
              "slot": "副将2",
              "name": "隆国"
            },
            {
              "slot": "補佐1",
              "name": "漂"
            },
            {
              "slot": "補佐2",
              "name": "姜燕"
            }
          ],
          "selfSlot": "副将2",
          "focusTitles": [
            "高威力の戦威ダメージを与える編成",
            "前列で使うのがおすすめ",
            "横列の味方を強化する編成"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦千伝・攻戦",
              "effect": "編成した部隊の攻撃+1% 編成した部隊の戦威+1%"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "城砦伍伝・防",
              "effect": "編成した部隊の防御+0.4%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            }
          ],
          "tipTitles": [
            "城砦千伝・攻戦がおすすめ",
            "補佐なら城砦伍伝にする"
          ]
        }
      },
      "skills": [
        "聡明",
        "開智",
        "守壁",
        "助勢"
      ],
      "personalities": [
        "支援",
        "馬陽の戦い",
        "王騎軍",
        "基本戦術",
        "教育係"
      ],
      "chainBase": 0.2,
      "attack": 551,
      "defense": 599,
      "war": 640,
      "strategy": 591
    },
    {
      "id": 28,
      "name": "魏加",
      "rarity": "SSR",
      "tenpu": 800,
      "type": "妨",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519295",
      "sourceId": "519295",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_28.png",
      "battleArtName": "落鳥一矢",
      "battleArtEffects": [
        "攻撃対象と同じ縦列の部隊に攻撃速度低下20％を付与",
        "攻撃対象に戦威150％の攻撃"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "戦法で敵を妨害する武将",
          "摎や王賁と同じ技能を持つ"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "介子坊"
            },
            {
              "slot": "主将",
              "name": "姜燕"
            },
            {
              "slot": "副将2",
              "name": "魏加"
            },
            {
              "slot": "補佐1",
              "name": "楚水"
            },
            {
              "slot": "補佐2",
              "name": "昌文君"
            }
          ],
          "selfSlot": "副将2",
          "focusTitles": [
            "戦法で大ダメージを与える編成",
            "軍勢の後列で使うのがおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦千伝・戦",
              "effect": "編成した部隊の戦威+1%"
            },
            {
              "name": "城砦千伝・攻戦",
              "effect": "編成した部隊の攻撃+1% 編成した部隊の戦威+1%"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            },
            {
              "name": "減戦短縮の印",
              "effect": "習得した武将の戦威+4"
            }
          ],
          "tipTitles": [
            "城砦千伝・戦がおすすめ",
            "秘伝で部隊の戦威を上げよう"
          ]
        }
      },
      "skills": [
        "尖撃",
        "練武",
        "後備",
        "堅実"
      ],
      "personalities": [
        "術策",
        "馬陽の戦い",
        "趙軍",
        "弓の名手",
        "暗躍",
        "武人"
      ],
      "chainBase": 0.2,
      "attack": 623,
      "defense": 431,
      "war": 607,
      "strategy": 503
    },
    {
      "id": 19,
      "name": "鱗坊",
      "rarity": "SSR",
      "tenpu": 800,
      "type": "妨",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519304",
      "sourceId": "519304",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_19.png",
      "battleArtName": "悪罵挑発",
      "battleArtEffects": [
        "自部隊の防御を15％上昇",
        "攻撃対象と同じ縦列の部隊の防御、策略を30％低下"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "早い",
      "guide": {
        "evaluationPoints": [
          "耐久性能に優れる妨タイプ武将",
          "補佐として使うのがおすすめ"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "宮元"
            },
            {
              "slot": "主将",
              "name": "王翦"
            },
            {
              "slot": "副将2",
              "name": "漂"
            },
            {
              "slot": "補佐1",
              "name": "鱗坊"
            },
            {
              "slot": "補佐2",
              "name": "隆国"
            }
          ],
          "selfSlot": "補佐1",
          "focusTitles": [
            "戦法で敵を妨害する高耐久の編成",
            "基本陣や錐行陣の前列がおすすめ",
            "シーズン1の序盤用編成"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦千伝・攻戦",
              "effect": "編成した部隊の攻撃+1% 編成した部隊の戦威+1%"
            },
            {
              "name": "城砦千伝・戦",
              "effect": "編成した部隊の戦威+1%"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "城砦伍伝・防",
              "effect": "編成した部隊の防御+0.4%"
            }
          ],
          "tipTitles": [
            "城砦千伝・攻戦がおすすめ",
            "補佐なら城砦伍伝にする"
          ]
        }
      },
      "skills": [
        "頑健",
        "開智",
        "献策",
        "豪傑"
      ],
      "personalities": [
        "防御",
        "馬陽の戦い",
        "王騎軍",
        "武人",
        "慎重"
      ],
      "chainBase": 0.2,
      "attack": 655,
      "defense": 687,
      "war": 566,
      "strategy": 471
    },
    {
      "id": 20,
      "name": "黄離弦",
      "rarity": "SSR",
      "tenpu": 800,
      "type": "闘",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519303",
      "sourceId": "519303",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_20.png",
      "battleArtName": "強弓一矢",
      "battleArtEffects": [
        "自部隊に会心発生上昇20％を付与",
        "攻撃対象に戦威150％の攻撃（自部隊が後列にいる場合、250％に威力上昇）"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "遅い",
      "guide": {
        "evaluationPoints": [
          "後列で性能を発揮する武将",
          "救援武将としても優秀"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "宮元"
            },
            {
              "slot": "主将",
              "name": "呉慶"
            },
            {
              "slot": "副将2",
              "name": "黄離弦"
            },
            {
              "slot": "補佐1",
              "name": "魏加"
            },
            {
              "slot": "補佐2",
              "name": "河了貂"
            }
          ],
          "selfSlot": "副将2",
          "focusTitles": [
            "反撃で大ダメージを与える編成",
            "軍勢の後列で使うのがおすすめ",
            "通常攻撃で火力を出す編成"
          ]
        },
        "recommendedSecrets": {
          "items": [
            {
              "name": "城砦千伝・戦",
              "effect": "編成した部隊の戦威+1%"
            },
            {
              "name": "城砦千伝・攻戦",
              "effect": "編成した部隊の攻撃+1% 編成した部隊の戦威+1%"
            },
            {
              "name": "城砦百伝・攻",
              "effect": "編成した部隊の攻撃+1%"
            },
            {
              "name": "城砦伍伝・攻",
              "effect": "編成した部隊の攻撃+0.4%"
            },
            {
              "name": "城砦伍伝・戦",
              "effect": "編成した部隊の戦威+0.4%"
            }
          ],
          "tipTitles": [
            "城砦千伝・戦がおすすめ",
            "補佐なら城砦伍伝にする"
          ]
        }
      },
      "skills": [
        "発揚",
        "練武",
        "後備",
        "防陣"
      ],
      "personalities": [
        "殲滅",
        "蛇甘平原の戦い",
        "魏軍",
        "弓の名手",
        "武人"
      ],
      "chainBase": 0.2,
      "attack": 640,
      "defense": 543,
      "war": 519,
      "strategy": 447
    },
    {
      "id": 44,
      "name": "カク備",
      "rarity": "SR",
      "tenpu": 650,
      "type": "援",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519279",
      "sourceId": "519279",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_44.png",
      "battleArtName": "俊英立身",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の対物を30％上昇",
        "自部隊の攻撃を10％上昇"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "味方の対物を強化する武将",
          "調達隊でも活躍できる"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "壁"
            },
            {
              "slot": "主将",
              "name": "カク備"
            },
            {
              "slot": "副将2",
              "name": "楚水"
            },
            {
              "slot": "補佐1",
              "name": "タジフ"
            },
            {
              "slot": "補佐2",
              "name": "ムタ"
            }
          ],
          "selfSlot": "主将",
          "focusTitles": [
            "王騎の怪鳥を意識した編成",
            "前列で使うのがおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [],
          "tipTitles": []
        }
      },
      "skills": [
        "攻城",
        "人望"
      ],
      "personalities": [
        "支援",
        "山陽の戦い",
        "真面目",
        "基本戦術",
        "器用"
      ],
      "chainBase": 0.2,
      "attack": 493,
      "defense": 487,
      "war": 461,
      "strategy": 493
    },
    {
      "id": 47,
      "name": "丁之",
      "rarity": "SR",
      "tenpu": 650,
      "type": "援",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519276",
      "sourceId": "519276",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_47.png",
      "battleArtName": "尽忠猛進",
      "battleArtEffects": [
        "自部隊と同じ縦列の部隊の攻撃を10％上昇（自部隊の兵力が50％以下の場合、30％に効果上昇）"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "技能で攻撃を上昇させる武将",
          "戦法も単純ながら強力"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "丁之"
            },
            {
              "slot": "主将",
              "name": "干央"
            },
            {
              "slot": "副将2",
              "name": "来輝"
            },
            {
              "slot": "補佐1",
              "name": "中鉄"
            },
            {
              "slot": "補佐2",
              "name": "崇原"
            }
          ],
          "selfSlot": "副将1",
          "focusTitles": [
            "火力に特化した序盤用の編成",
            "楚水を主将に編成するのもあり"
          ]
        },
        "recommendedSecrets": {
          "items": [],
          "tipTitles": []
        }
      },
      "skills": [
        "鋭鋒",
        "練武"
      ],
      "personalities": [
        "支援",
        "馬陽の戦い",
        "補佐役",
        "武人"
      ],
      "chainBase": 0.2,
      "attack": 422,
      "defense": 435,
      "war": 461,
      "strategy": 396
    },
    {
      "id": 35,
      "name": "中鉄",
      "rarity": "SR",
      "tenpu": 650,
      "type": "闘",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519288",
      "sourceId": "519288",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_35.png",
      "battleArtName": "富村の殺し屋",
      "battleArtEffects": [
        "自部隊の攻撃を20％上昇",
        "攻撃対象に戦威150％の攻撃（自部隊が前列にいる場合、200％に威力上昇）"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "扱いやすい技能を持つ武将",
          "序盤は信の副将としても使える"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "漂"
            },
            {
              "slot": "主将",
              "name": "信"
            },
            {
              "slot": "副将2",
              "name": "中鉄"
            },
            {
              "slot": "補佐1",
              "name": "丁之"
            },
            {
              "slot": "補佐2",
              "name": "崇原"
            }
          ],
          "selfSlot": "副将2",
          "focusTitles": [
            "敵単体に大ダメージを与える編成",
            "前列で使うのがおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [],
          "tipTitles": []
        }
      },
      "skills": [
        "鋭鋒",
        "練武"
      ],
      "personalities": [
        "殲滅",
        "馬陽の戦い",
        "飛信隊",
        "怪力"
      ],
      "chainBase": 0.2,
      "attack": 461,
      "defense": 357,
      "war": 317,
      "strategy": 272
    },
    {
      "id": 40,
      "name": "去亥",
      "rarity": "SR",
      "tenpu": 650,
      "type": "妨",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519283",
      "sourceId": "519283",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_40.png",
      "battleArtName": "突撃の怒号",
      "battleArtEffects": [
        "攻撃対象と同じ横列の部隊の防御を25％低下",
        "自部隊の戦威を10％上昇"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "早い",
      "guide": {
        "evaluationPoints": [
          "発動の早い防御デバフ戦法",
          "飛信隊編成で活躍"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "去亥"
            },
            {
              "slot": "主将",
              "name": "信"
            },
            {
              "slot": "副将2",
              "name": "漂"
            },
            {
              "slot": "補佐1",
              "name": "中鉄"
            },
            {
              "slot": "補佐2",
              "name": "丁之"
            }
          ],
          "selfSlot": "副将1",
          "focusTitles": [
            "戦法で敵部隊の兵力を削る編成",
            "会心編成の補佐で使うのもあり"
          ]
        },
        "recommendedSecrets": {
          "items": [],
          "tipTitles": []
        }
      },
      "skills": [
        "豪傑",
        "統制"
      ],
      "personalities": [
        "支援",
        "馬陽の戦い",
        "飛信隊",
        "荒くれ者"
      ],
      "chainBase": 0.2,
      "attack": 447,
      "defense": 467,
      "war": 305,
      "strategy": 266
    },
    {
      "id": 36,
      "name": "崇原",
      "rarity": "SR",
      "tenpu": 650,
      "type": "闘",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519287",
      "sourceId": "519287",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_36.png",
      "battleArtName": "剣閃一薙",
      "battleArtEffects": [
        "自部隊に会心威力上昇20％を付与",
        "攻撃対象に戦威200％の攻撃"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "戦法攻撃が戦威200％と高威力"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "バジオウ"
            },
            {
              "slot": "主将",
              "name": "楊端和"
            },
            {
              "slot": "副将2",
              "name": "崇原"
            },
            {
              "slot": "補佐1",
              "name": "中鉄"
            },
            {
              "slot": "補佐2",
              "name": "丁之"
            }
          ],
          "selfSlot": "副将2",
          "focusTitles": [
            "戦法と会心でダメージを与える編成",
            "信や沛浪と組み合わせるのもおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [],
          "tipTitles": []
        }
      },
      "skills": [
        "掃討",
        "練武"
      ],
      "personalities": [
        "殲滅",
        "馬陽の戦い",
        "飛信隊",
        "剣客",
        "教育係"
      ],
      "chainBase": 0.2,
      "attack": 526,
      "defense": 441,
      "war": 428,
      "strategy": 343
    },
    {
      "id": 46,
      "name": "来輝",
      "rarity": "SR",
      "tenpu": 650,
      "type": "闘",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519277",
      "sourceId": "519277",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_46.png",
      "battleArtName": "万死一生",
      "battleArtEffects": [
        "自部隊の防御を10％上昇",
        "攻撃対象と同じ縦列の部隊に戦威150％の攻撃"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "戦法で防御上昇+縦列に攻撃",
          "個性「武人」持ちは闘タイプが多い"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "来輝"
            },
            {
              "slot": "主将",
              "name": "縛虎申"
            },
            {
              "slot": "副将2",
              "name": "信"
            },
            {
              "slot": "補佐1",
              "name": "丁之"
            },
            {
              "slot": "補佐2",
              "name": "崇原"
            }
          ],
          "selfSlot": "副将1",
          "focusTitles": [
            "敵の兵力や建物の耐久を削る編成",
            "主将は干央でも代用できる"
          ]
        },
        "recommendedSecrets": {
          "items": [],
          "tipTitles": []
        }
      },
      "skills": [
        "奮戦",
        "糧道"
      ],
      "personalities": [
        "突撃",
        "馬陽の戦い",
        "補佐役",
        "武人"
      ],
      "chainBase": 0.2,
      "attack": 455,
      "defense": 480,
      "war": 447,
      "strategy": 415
    },
    {
      "id": 38,
      "name": "松左",
      "rarity": "SR",
      "tenpu": 650,
      "type": "援",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519285",
      "sourceId": "519285",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_38.png",
      "battleArtName": "自在なる槍",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の弱化効果を1つ解除",
        "攻撃対象に150％の攻撃"
      ],
      "battleArtType": "戦威",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "弱化解除が行える攻撃戦法",
          "飛信隊編成で活躍"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "漂"
            },
            {
              "slot": "主将",
              "name": "信"
            },
            {
              "slot": "副将2",
              "name": "松左"
            },
            {
              "slot": "補佐1",
              "name": "中鉄"
            },
            {
              "slot": "補佐2",
              "name": "丁之"
            }
          ],
          "selfSlot": "副将2",
          "focusTitles": [
            "戦法でデバフを解除する編成",
            "陣形の前列で使うのがおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [],
          "tipTitles": []
        }
      },
      "skills": [
        "猛者",
        "開智"
      ],
      "personalities": [
        "支援",
        "馬陽の戦い",
        "飛信隊",
        "槍名人"
      ],
      "chainBase": 0.2,
      "attack": 467,
      "defense": 480,
      "war": 441,
      "strategy": 435
    },
    {
      "id": 33,
      "name": "沛浪",
      "rarity": "SR",
      "tenpu": 650,
      "type": "援",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519290",
      "sourceId": "519290",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_33.png",
      "battleArtName": "古強者の戦",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊に会心発生上昇15％を付与"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "戦法の会心発生上昇が優秀",
          "変換できる秘伝も強力"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "沛浪"
            },
            {
              "slot": "主将",
              "name": "信"
            },
            {
              "slot": "副将2",
              "name": "竜川"
            },
            {
              "slot": "補佐1",
              "name": "中鉄"
            },
            {
              "slot": "補佐2",
              "name": "丁之"
            }
          ],
          "selfSlot": "副将1",
          "focusTitles": [
            "戦法と会心でダメージを与える編成",
            "補佐の技能で会心を補うのもあり"
          ]
        },
        "recommendedSecrets": {
          "items": [],
          "tipTitles": []
        }
      },
      "skills": [
        "豪傑",
        "統制"
      ],
      "personalities": [
        "主導",
        "馬陽の戦い",
        "飛信隊",
        "荒くれ者",
        "経験豊富"
      ],
      "chainBase": 0.2,
      "attack": 415,
      "defense": 441,
      "war": 376,
      "strategy": 390
    },
    {
      "id": 34,
      "name": "田有",
      "rarity": "SR",
      "tenpu": 650,
      "type": "闘",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519289",
      "sourceId": "519289",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_34.png",
      "battleArtName": "巨漢の一撃",
      "battleArtEffects": [
        "自部隊の攻撃、防御を10％上昇",
        "攻撃対象に対物200％の攻撃"
      ],
      "battleArtType": "対物",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "対物に特化した武将",
          "飛信隊編成で活躍"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "カク備"
            },
            {
              "slot": "主将",
              "name": "楚水"
            },
            {
              "slot": "副将2",
              "name": "田有"
            },
            {
              "slot": "補佐1",
              "name": "崇原"
            },
            {
              "slot": "補佐2",
              "name": "中鉄"
            }
          ],
          "selfSlot": "副将2",
          "focusTitles": [
            "対物性能に特化した序盤用の編成",
            "信を主将に編成するのもあり"
          ]
        },
        "recommendedSecrets": {
          "items": [],
          "tipTitles": []
        }
      },
      "skills": [
        "攻城",
        "糧道"
      ],
      "personalities": [
        "防御",
        "馬陽の戦い",
        "飛信隊",
        "怪力",
        "器用"
      ],
      "chainBase": 0.2,
      "attack": 526,
      "defense": 467,
      "war": 350,
      "strategy": 376
    },
    {
      "id": 37,
      "name": "田永",
      "rarity": "SR",
      "tenpu": 650,
      "type": "妨",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519286",
      "sourceId": "519286",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_37.png",
      "battleArtName": "荒くれの罵声",
      "battleArtEffects": [
        "攻撃対象と同じ横列の部隊の攻撃、戦威を15％低下"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "早い",
      "guide": {
        "evaluationPoints": [
          "攻撃と戦威を下げる戦法",
          "飛信隊編成で活躍"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "田永"
            },
            {
              "slot": "主将",
              "name": "渕"
            },
            {
              "slot": "副将2",
              "name": "楚水"
            },
            {
              "slot": "補佐1",
              "name": "竜川"
            },
            {
              "slot": "補佐2",
              "name": "崇原"
            }
          ],
          "selfSlot": "副将1",
          "focusTitles": [
            "反撃でダメージを与える編成",
            "陣形の前列で使うのがおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [],
          "tipTitles": []
        }
      },
      "skills": [
        "頑健",
        "統制"
      ],
      "personalities": [
        "突撃",
        "馬陽の戦い",
        "飛信隊",
        "荒くれ者",
        "激情家"
      ],
      "chainBase": 0.2,
      "attack": 506,
      "defense": 487,
      "war": 382,
      "strategy": 357
    },
    {
      "id": 42,
      "name": "番陽",
      "rarity": "SR",
      "tenpu": 650,
      "type": "妨",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519281",
      "sourceId": "519281",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_42.png",
      "battleArtName": "大喝一声",
      "battleArtEffects": [
        "攻撃対象と同じ横列の部隊の攻撃を10％低下させ、攻撃速度低下15％を付与"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "戦法で敵の火力を抑える武将",
          "施設や商店で入手できる"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "丁之"
            },
            {
              "slot": "主将",
              "name": "昌文君"
            },
            {
              "slot": "副将2",
              "name": "番陽"
            },
            {
              "slot": "補佐1",
              "name": "中鉄"
            },
            {
              "slot": "補佐2",
              "name": "田永"
            }
          ],
          "selfSlot": "副将2",
          "focusTitles": [
            "反撃で敵にダメージを与える編成",
            "軍勢の後列で使うのがおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [],
          "tipTitles": []
        }
      },
      "skills": [
        "頑健",
        "統制"
      ],
      "personalities": [
        "支援",
        "山陽の戦い",
        "補佐役",
        "経験豊富",
        "玉鳳隊"
      ],
      "chainBase": 0.2,
      "attack": 422,
      "defense": 461,
      "war": 441,
      "strategy": 382
    },
    {
      "id": 41,
      "name": "石",
      "rarity": "SR",
      "tenpu": 650,
      "type": "護",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519282",
      "sourceId": "519282",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_41.png",
      "battleArtName": "青石族の笛",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の策略を15％上昇（自部隊が中列にいる場合、30％に効果上昇）",
        "自部隊と同じ横列の部隊の防御を15％上昇"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "早い",
      "guide": {
        "evaluationPoints": [
          "耐久面に優れた戦法を持つ",
          "中列で使うのがおすすめ"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "石"
            },
            {
              "slot": "主将",
              "name": "渕"
            },
            {
              "slot": "副将2",
              "name": "来輝"
            },
            {
              "slot": "補佐1",
              "name": "田永"
            },
            {
              "slot": "補佐2",
              "name": "丁之"
            }
          ],
          "selfSlot": "副将1",
          "focusTitles": [
            "反撃で敵にダメージを与える編成",
            "軍勢の中列で使うのがおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [],
          "tipTitles": []
        }
      },
      "skills": [
        "聡明",
        "糧道"
      ],
      "personalities": [
        "術策",
        "馬陽の戦い",
        "飛信隊",
        "器用"
      ],
      "chainBase": 0.2,
      "attack": 357,
      "defense": 435,
      "war": 363,
      "strategy": 441
    },
    {
      "id": 39,
      "name": "竜川",
      "rarity": "SR",
      "tenpu": 650,
      "type": "護",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519284",
      "sourceId": "519284",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_39.png",
      "battleArtName": "三倍力",
      "battleArtEffects": [
        "自部隊と同じ縦列の部隊の防御を15％上昇",
        "自部隊の攻撃、対物を10％上昇"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "普通",
      "guide": {
        "evaluationPoints": [
          "ステータスと戦法が優秀",
          "信の副将として最適"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "竜川"
            },
            {
              "slot": "主将",
              "name": "信"
            },
            {
              "slot": "副将2",
              "name": "縛虎申"
            },
            {
              "slot": "補佐1",
              "name": "田有"
            },
            {
              "slot": "補佐2",
              "name": "崇原"
            }
          ],
          "selfSlot": "副将1",
          "focusTitles": [
            "軍陣や詰所を攻撃する軍団戦用の編成",
            "縛虎申は飛信隊の武将で代用可能"
          ]
        },
        "recommendedSecrets": {
          "items": [],
          "tipTitles": []
        }
      },
      "skills": [
        "威風",
        "糧道"
      ],
      "personalities": [
        "突撃",
        "馬陽の戦い",
        "飛信隊",
        "怪力",
        "慎重"
      ],
      "chainBase": 0.2,
      "attack": 538,
      "defense": 461,
      "war": 350,
      "strategy": 357
    },
    {
      "id": 45,
      "name": "肆氏",
      "rarity": "SR",
      "tenpu": 650,
      "type": "妨",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519278",
      "sourceId": "519278",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_45.png",
      "battleArtName": "深謀暗躍",
      "battleArtEffects": [
        "攻撃対象と同じ横列の部隊の防御を20％、策略を30％低下"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "早い",
      "guide": {
        "evaluationPoints": [
          "戦法で攻撃対象+横列の防御と策略を低下",
          "成蟜一派と相性がいい"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "肆氏"
            },
            {
              "slot": "主将",
              "name": "成蟜"
            },
            {
              "slot": "副将2",
              "name": "ランカイ"
            },
            {
              "slot": "補佐1",
              "name": "石"
            },
            {
              "slot": "補佐2",
              "name": "尚鹿"
            }
          ],
          "selfSlot": "副将1",
          "focusTitles": [
            "複数のデバフで敵を妨害する編成",
            "中列で使うのがおすすめ"
          ]
        },
        "recommendedSecrets": {
          "items": [],
          "tipTitles": []
        }
      },
      "skills": [
        "聡明",
        "開智"
      ],
      "personalities": [
        "防御",
        "王弟反乱",
        "成蟜一派",
        "政権闘争",
        "謀略家"
      ],
      "chainBase": 0.2,
      "attack": 331,
      "defense": 396,
      "war": 422,
      "strategy": 577
    },
    {
      "id": 43,
      "name": "胡漸",
      "rarity": "SR",
      "tenpu": 650,
      "type": "護",
      "sourceUrl": "https://gamewith.jp/kingdom-hadou/519280",
      "sourceId": "519280",
      "imageUrl": "https://img.gamewith.jp/article_tools/kingdom-hadou/gacha/chara_43.png",
      "battleArtName": "老将の妙案",
      "battleArtEffects": [
        "自部隊と同じ横列の部隊の戦威、策略を10％上昇",
        "（自部隊の兵力が50％以下の場合、30％に効果上昇）"
      ],
      "battleArtType": "補助",
      "battleArtChainOrder": "早い",
      "guide": {
        "evaluationPoints": [
          "発動の早いバフ戦法",
          "施設や商店で入手できる"
        ],
        "latestFormation": {
          "placements": [
            {
              "slot": "副将1",
              "name": "シュンメン"
            },
            {
              "slot": "主将",
              "name": "楊端和"
            },
            {
              "slot": "副将2",
              "name": "バジオウ"
            },
            {
              "slot": "補佐1",
              "name": "鱗坊"
            },
            {
              "slot": "補佐2",
              "name": "胡漸"
            }
          ],
          "selfSlot": "補佐2",
          "focusTitles": [
            "恐怖で妨害しつつ兵力を削る編成",
            "どの列に配置しても活躍できる"
          ]
        },
        "recommendedSecrets": {
          "items": [],
          "tipTitles": []
        }
      },
      "skills": [
        "献策",
        "統制"
      ],
      "personalities": [
        "防御",
        "山陽の戦い",
        "教育係",
        "補佐役",
        "経験豊富",
        "楽華隊"
      ],
      "chainBase": 0.2,
      "attack": 402,
      "defense": 467,
      "war": 376,
      "strategy": 396
    }
  ],
  "skills": {
    "威風": {
      "name": "威風",
      "order": 0,
      "summary": "部隊の兵力が上昇",
      "level": 5,
      "initialEffect": "部隊の兵力+3%",
      "maxEffect": "部隊の兵力+20%（Lv5）",
      "conditions": []
    },
    "鋭鋒": {
      "name": "鋭鋒",
      "order": 1,
      "summary": "部隊の攻撃が上昇",
      "level": 5,
      "initialEffect": "部隊の攻撃+3%",
      "maxEffect": "部隊の攻撃+20%（Lv5）",
      "conditions": []
    },
    "頑健": {
      "name": "頑健",
      "order": 2,
      "summary": "部隊の防御が上昇",
      "level": 5,
      "initialEffect": "部隊の防御+3%",
      "maxEffect": "部隊の防御+20%（Lv5）",
      "conditions": []
    },
    "尖撃": {
      "name": "尖撃",
      "order": 3,
      "summary": "部隊の戦威が上昇",
      "level": 5,
      "initialEffect": "部隊の戦威+3%",
      "maxEffect": "部隊の戦威+20%（Lv5）",
      "conditions": []
    },
    "聡明": {
      "name": "聡明",
      "order": 4,
      "summary": "部隊の策略が上昇",
      "level": 5,
      "initialEffect": "部隊の策略+3%",
      "maxEffect": "部隊の策略+20%（Lv5）",
      "conditions": []
    },
    "掃討": {
      "name": "掃討",
      "order": 5,
      "summary": "部隊の攻撃速度が上昇",
      "level": 5,
      "initialEffect": "部隊の攻撃速度+2%",
      "maxEffect": "部隊の攻撃速度+15%（Lv5）",
      "conditions": []
    },
    "猛者": {
      "name": "猛者",
      "order": 6,
      "summary": "部隊の会心発生率が上昇",
      "level": 5,
      "initialEffect": "部隊の会心発生率+2%",
      "maxEffect": "部隊の会心発生率+15%（Lv5）",
      "conditions": []
    },
    "豪傑": {
      "name": "豪傑",
      "order": 7,
      "summary": "部隊の会心威力が上昇",
      "level": 5,
      "initialEffect": "部隊の会心威力+4%",
      "maxEffect": "部隊の会心威力+25%（Lv5）",
      "conditions": []
    },
    "攻城": {
      "name": "攻城",
      "order": 8,
      "summary": "部隊の対物が上昇",
      "level": 5,
      "initialEffect": "部隊の対物+3%",
      "maxEffect": "部隊の対物+20%（Lv5）",
      "conditions": []
    },
    "堅実": {
      "name": "堅実",
      "order": 9,
      "summary": "攻撃を受けた際、負傷兵として生存する兵力が増加",
      "level": 5,
      "initialEffect": "攻撃を受けた際、負傷兵として生存する兵力+10%",
      "maxEffect": "攻撃を受けた際、負傷兵として生存する兵力+30%（Lv5）",
      "conditions": []
    },
    "攻陣": {
      "name": "攻陣",
      "order": 10,
      "summary": "自身が陣形の前列にいる場合、部隊の攻撃が上昇",
      "level": 5,
      "initialEffect": "自身が陣形の前列にいる場合、部隊の攻撃+4%",
      "maxEffect": "自身が陣形の前列にいる場合、部隊の攻撃+25%（Lv5）",
      "conditions": [
        "front"
      ]
    },
    "守壁": {
      "name": "守壁",
      "order": 11,
      "summary": "自身が陣形の前列にいる場合、部隊の防御が上昇",
      "level": 5,
      "initialEffect": "自身が陣形の前列にいる場合、部隊の防御+4%",
      "maxEffect": "自身が陣形の前列にいる場合、部隊の防御+25%（Lv5）",
      "conditions": [
        "front"
      ]
    },
    "破壁": {
      "name": "破壁",
      "order": 12,
      "summary": "自身が陣形の前列にいる場合、部隊の対物が上昇",
      "level": 5,
      "initialEffect": "自身が陣形の前列にいる場合、部隊の対物+4%",
      "maxEffect": "自身が陣形の前列にいる場合、部隊の対物+25%（Lv5）",
      "conditions": [
        "front"
      ]
    },
    "枢機": {
      "name": "枢機",
      "order": 13,
      "summary": "自身が陣形の中列にいる場合、部隊の攻撃が上昇",
      "level": 5,
      "initialEffect": "自身が陣形の中列にいる場合、部隊の攻撃+4%",
      "maxEffect": "自身が陣形の中列にいる場合、部隊の攻撃+25%（Lv5）",
      "conditions": [
        "middle"
      ]
    },
    "後備": {
      "name": "後備",
      "order": 14,
      "summary": "自身が陣形の後列にいる場合、部隊の攻撃が上昇",
      "level": 5,
      "initialEffect": "自身が陣形の後列にいる場合、部隊の攻撃+4%",
      "maxEffect": "自身が陣形の後列にいる場合、部隊の攻撃+25%（Lv5）",
      "conditions": [
        "back"
      ]
    },
    "防陣": {
      "name": "防陣",
      "order": 15,
      "summary": "自身が陣形の後列にいる場合、部隊の防御が上昇",
      "level": 5,
      "initialEffect": "自身が陣形の後列にいる場合、部隊の防御+4%",
      "maxEffect": "自身が陣形の後列にいる場合、部隊の防御+25%（Lv5）",
      "conditions": [
        "back"
      ]
    },
    "発揚": {
      "name": "発揚",
      "order": 16,
      "summary": "部隊の兵力が50%以上の際、部隊の攻撃、防御が上昇",
      "level": 5,
      "initialEffect": "部隊の兵力が50%以上の際、部隊の攻撃、防御+2%",
      "maxEffect": "部隊の兵力が50%以上の際、部隊の攻撃、防御+15%（Lv5）",
      "conditions": []
    },
    "奮戦": {
      "name": "奮戦",
      "order": 17,
      "summary": "部隊の兵力が50%以下の際、部隊の攻撃が上昇",
      "level": 5,
      "initialEffect": "部隊の兵力が50%以下の際、部隊の攻撃+8%",
      "maxEffect": "部隊の兵力が50%以下の際、部隊の攻撃+35%（Lv5）",
      "conditions": []
    },
    "不屈": {
      "name": "不屈",
      "order": 18,
      "summary": "部隊の兵力が50%以下の際、部隊の防御が上昇",
      "level": 5,
      "initialEffect": "部隊の兵力が50%以下の際、部隊の防御+8%",
      "maxEffect": "部隊の兵力が50%以下の際、部隊の防御+35%（Lv5）",
      "conditions": []
    },
    "逆境": {
      "name": "逆境",
      "order": 19,
      "summary": "部隊の兵力が50%以下の際、部隊の戦威が上昇",
      "level": 5,
      "initialEffect": "部隊の兵力が50%以下の際、部隊の戦威+8%",
      "maxEffect": "部隊の兵力が50%以下の際、部隊の戦威+35%（Lv5）",
      "conditions": []
    },
    "衆兵": {
      "name": "衆兵",
      "order": 20,
      "summary": "自身が主将の場合、部隊の兵力が上昇",
      "level": 5,
      "initialEffect": "自身が主将の場合、部隊の兵力+4%",
      "maxEffect": "自身が主将の場合、部隊の兵力+25%（Lv5）",
      "conditions": [
        "main"
      ]
    },
    "大勇": {
      "name": "大勇",
      "order": 21,
      "summary": "自身が主将の場合、部隊の攻撃、戦威、対物が上昇",
      "level": 5,
      "initialEffect": "自身が主将の場合、部隊の攻撃、戦威、対物+4%",
      "maxEffect": "自身が主将の場合、部隊の攻撃、戦威、対物+25%（Lv5）",
      "conditions": [
        "main"
      ]
    },
    "献身": {
      "name": "献身",
      "order": 22,
      "summary": "自身が副将か補佐の場合、部隊の攻撃が上昇",
      "level": 5,
      "initialEffect": "自身が副将か補佐の場合、部隊の攻撃+4%",
      "maxEffect": "自身が副将か補佐の場合、部隊の攻撃+25%（Lv5）",
      "conditions": [
        "vice",
        "aide"
      ]
    },
    "扶助": {
      "name": "扶助",
      "order": 23,
      "summary": "自身が副将か補佐の場合、部隊の防御が上昇",
      "level": 5,
      "initialEffect": "自身が副将か補佐の場合、部隊の防御+4%",
      "maxEffect": "自身が副将か補佐の場合、部隊の防御+25%（Lv5）",
      "conditions": [
        "vice",
        "aide"
      ]
    },
    "助勢": {
      "name": "助勢",
      "order": 24,
      "summary": "自身が副将か補佐の場合、部隊の戦威が上昇",
      "level": 5,
      "initialEffect": "自身が副将か補佐の場合、部隊の戦威+4%",
      "maxEffect": "自身が副将か補佐の場合、部隊の戦威+25%（Lv5）",
      "conditions": [
        "vice",
        "aide"
      ]
    },
    "献策": {
      "name": "献策",
      "order": 25,
      "summary": "自身が副将か補佐の場合、部隊の策略が上昇",
      "level": 5,
      "initialEffect": "自身が副将か補佐の場合、部隊の策略+4%",
      "maxEffect": "自身が副将か補佐の場合、部隊の策略+25%（Lv5）",
      "conditions": [
        "vice",
        "aide"
      ]
    },
    "雄心": {
      "name": "雄心",
      "order": 26,
      "summary": "自身が主将の場合、自身と同じ個性を持つ武将の人数に応じて、部隊の攻撃、防御が上昇",
      "level": 3,
      "initialEffect": "自身が主将の場合、自身と同じ個性を持つ武将の人数×1%、部隊の攻撃、防御上昇",
      "maxEffect": "自身が主将の場合、自身と同じ個性を持つ武将の人数×3%、部隊の攻撃、防御上昇（Lv3）",
      "conditions": [
        "main"
      ]
    },
    "怪鳥": {
      "name": "怪鳥",
      "order": 27,
      "summary": "自身が主将の場合、陣形効果算出の際、闘タイプとしても扱う",
      "level": 1,
      "initialEffect": "自身が主将の場合、陣形効果算出の際 闘タイプとしても扱う",
      "maxEffect": "-",
      "conditions": [
        "main"
      ]
    },
    "炎将": {
      "name": "炎将",
      "order": 28,
      "summary": "自身が主将の場合、部隊の攻撃が上昇(部隊の炎状態で変化)",
      "level": 1,
      "initialEffect": "自身が主将の場合、部隊の攻撃+5% （自部隊が種火状態の場合5%、炎状態の場合10%、大炎状態の場合15%効果上昇）",
      "maxEffect": "-",
      "conditions": [
        "main"
      ]
    },
    "死王": {
      "name": "死王",
      "order": 29,
      "summary": "自身が主将の場合、部隊の会心発生率が上昇。交戦開始時、自身1部隊に攻撃速度上昇を付与",
      "level": 1,
      "initialEffect": "自身が主将の場合、部隊の会心発生率+10% 交戦開始時、自身1部隊に攻撃速度上昇20%を付与（15秒）",
      "maxEffect": "-",
      "conditions": [
        "main"
      ]
    },
    "戦士": {
      "name": "戦士",
      "order": 30,
      "summary": "自身が主将か、副将で主将と共通する個性が合計3個以上の場合、部隊の攻撃速度が上昇",
      "level": 1,
      "initialEffect": "自身が主将か、副将で主将と共通する個性が合計3個以上の場合、部隊の攻撃速度+10%",
      "maxEffect": "-",
      "conditions": [
        "main",
        "vice"
      ]
    },
    "腹心": {
      "name": "腹心",
      "order": 31,
      "summary": "自身が主将か、副将で主将と共通する個性が合計3個以上の場合、部隊の攻撃、防御、策略が上昇",
      "level": 1,
      "initialEffect": "自身が主将か、副将で主将と共通する個性が合計3個以上の場合、部隊の攻撃、防御、策略+3%",
      "maxEffect": "-",
      "conditions": [
        "main",
        "vice"
      ]
    },
    "糧道": {
      "name": "糧道",
      "order": 32,
      "summary": "",
      "level": 0,
      "initialEffect": "兵糧に任命した場合、任命効果+8%",
      "maxEffect": "兵糧に任命した場合、任命効果+24%（Lv3）",
      "conditions": []
    },
    "練武": {
      "name": "練武",
      "order": 33,
      "summary": "",
      "level": 0,
      "initialEffect": "武心に任命した場合、任命効果+8%",
      "maxEffect": "武心に任命した場合、任命効果+24%（Lv3）",
      "conditions": []
    },
    "統制": {
      "name": "統制",
      "order": 34,
      "summary": "",
      "level": 0,
      "initialEffect": "統心に任命した場合、任命効果+8%",
      "maxEffect": "統心に任命した場合、任命効果+24%（Lv3）",
      "conditions": []
    },
    "開智": {
      "name": "開智",
      "order": 35,
      "summary": "",
      "level": 0,
      "initialEffect": "智心に任命した場合、任命効果+8%",
      "maxEffect": "智心に任命した場合、任命効果+24%（Lv3）",
      "conditions": []
    },
    "人望": {
      "name": "人望",
      "order": 36,
      "summary": "",
      "level": 0,
      "initialEffect": "調達隊に編成した場合、部隊の魅力+4%",
      "maxEffect": "調達隊に編成した場合、部隊の魅力+11%（Lv3）",
      "conditions": []
    },
    "洞察": {
      "name": "洞察",
      "order": 37,
      "summary": "",
      "level": 0,
      "initialEffect": "調達の報酬が5%の確率で、100%上昇",
      "maxEffect": "調達の報酬が15%の確率で、100%上昇（Lv3）",
      "conditions": []
    },
    "報復": {
      "name": "報復",
      "order": 38,
      "summary": "自身が主将の場合、反撃による与ダメージが上昇。反撃時、反撃対象の1部隊の攻撃を低下させる",
      "level": 1,
      "initialEffect": "自身が主将の場合、反撃による与ダメージ+100% 反撃時、反撃対象の1部隊の攻撃を10%低下(6秒)",
      "maxEffect": "-",
      "conditions": [
        "main"
      ]
    },
    "飛槍": {
      "name": "飛槍",
      "order": 39,
      "summary": "自身が主将か、副将で主将と共通する個性が合計3個以上の場合、部隊の攻撃速度、策略が上昇",
      "level": 1,
      "initialEffect": "自身が主将か、副将で主将と共通する個性が合計3個以上の場合、部隊の攻撃速度、策略+5％",
      "maxEffect": "-",
      "conditions": [
        "main",
        "vice"
      ]
    },
    "俊才": {
      "name": "俊才",
      "order": 40,
      "summary": "自身が主将の際、部隊の防御、戦威、策略が上昇",
      "level": 5,
      "initialEffect": "自身が主将の際、部隊の防御、戦威、策略+4%",
      "maxEffect": "自身が主将の際、部隊の防御、戦威、策略+25%（Lv5）",
      "conditions": [
        "main"
      ]
    },
    "白老": {
      "name": "白老",
      "order": 41,
      "summary": "自身が主将か、副将で主将と共通する個性が合計2個以上の場合、交戦開始時、自部隊に堅固を付与",
      "level": 1,
      "initialEffect": "自身が主将か、副将で主将と共通する個性が合計2個以上の場合、交戦開始時、自部隊に堅固が付与されていなければ、堅固5%を付与(10秒)",
      "maxEffect": "-",
      "conditions": [
        "main",
        "vice"
      ]
    },
    "戦神子": {
      "name": "戦神子",
      "order": 42,
      "summary": "自身が主将で、副将と共通する個性が合計3個以上の場合、部隊の戦威が上昇、軍勢内全部隊の機動が上昇",
      "level": 1,
      "initialEffect": "自身が主将で、副将と共通する個性が合計3個以上の場合、部隊の戦威+10% 軍勢内全部隊の機動+10%",
      "maxEffect": "-",
      "conditions": [
        "main",
        "vice"
      ]
    },
    "伝熱": {
      "name": "伝熱",
      "order": 43,
      "summary": "自身が主将の場合、交戦開始時、軍勢内の攻撃速度上昇が付与されていない部隊に攻撃速度上昇を付与",
      "level": 1,
      "initialEffect": "自身が主将の場合、交戦開始時、軍勢内の攻撃速度上昇が付与されていない部隊に攻撃速度上昇10%を付与（10秒）",
      "maxEffect": "-",
      "conditions": [
        "main"
      ]
    },
    "不敗": {
      "name": "不敗",
      "order": 44,
      "summary": "自身が主将か、護タイプ部隊の副将の場合、攻撃対象以外からの被ダメージを軽減\n（自身が主将の場合、軽減効果が強化される）",
      "level": 1,
      "initialEffect": "自身が主将か、護タイプ部隊の副将の場合、攻撃対象以外からの被ダメージ₋3% （自身が主将の場合、さらに₋7%）",
      "maxEffect": "-",
      "conditions": [
        "main",
        "vice"
      ]
    },
    "山読": {
      "name": "山読",
      "order": 45,
      "summary": "自身が主将の場合、自身と同じ個性を持つ武将の人数に応じて、部隊の防御と対物が上昇",
      "level": 1,
      "initialEffect": "自身が主将の場合、自身と同じ個性を持つ武将の人数×1.5%、部隊の防御と対物が上昇",
      "maxEffect": "-",
      "conditions": [
        "main"
      ]
    },
    "巨人": {
      "name": "巨人",
      "order": 46,
      "summary": "自身が主将の場合、交戦開始時、自部隊に攻撃上昇が付与されていなければ、自身1部隊に攻撃上昇を付与",
      "level": 1,
      "initialEffect": "自身が主将の場合、交戦開始時、自部隊に攻撃上昇が付与されていなければ、自身1部隊に攻撃上昇15％を付与（25秒）",
      "maxEffect": "-",
      "conditions": [
        "main"
      ]
    },
    "才賢": {
      "name": "才賢",
      "order": 47,
      "summary": "自紙が主将の場合、交戦開始時、攻撃対象の部隊に攻撃速度低下が付与されていなければ、攻撃速度低下を付与",
      "level": 1,
      "initialEffect": "自紙が主将の場合、交戦開始時、攻撃対象の部隊に攻撃速度低下が付与されていなければ、攻撃速度低下50％を付与（12秒）",
      "maxEffect": "-",
      "conditions": [
        "main"
      ]
    },
    "黒弓": {
      "name": "黒弓",
      "order": 48,
      "summary": "交戦開始時、攻撃対象の部隊に\n病毒\nが付与されていなければ\n病毒\nを付与",
      "level": 1,
      "initialEffect": "交戦開始時、攻撃対象の部隊に 病毒 が付与されていなければ、 病毒 3％を付与（10秒）",
      "maxEffect": "-",
      "conditions": []
    },
    "奇矯": {
      "name": "奇矯",
      "order": 49,
      "summary": "交戦開始時、攻撃対象の部隊に攻撃低下が付与されていなければ攻撃低下を付与",
      "level": 1,
      "initialEffect": "交戦開始時、攻撃対象の部隊に攻撃低下が付与されていなければ、攻撃低下15％を付与（12秒）",
      "maxEffect": "-",
      "conditions": []
    },
    "堅靭": {
      "name": "堅靭",
      "order": 50,
      "summary": "自身が主将か副将の場合、自部隊の兵力が30％以下になった際に1度だけ、自身1部隊に通常攻撃回避を付与",
      "level": 1,
      "initialEffect": "自身が主将か副将の場合、自部隊の兵力が30％以下になった際に1度だけ、自身1部隊に通常攻撃回避100％を付与（10秒）",
      "maxEffect": "-",
      "conditions": [
        "main",
        "vice"
      ]
    }
  },
  "skillOrder": [
    "威風",
    "鋭鋒",
    "頑健",
    "尖撃",
    "聡明",
    "掃討",
    "猛者",
    "豪傑",
    "攻城",
    "堅実",
    "攻陣",
    "守壁",
    "破壁",
    "枢機",
    "後備",
    "防陣",
    "発揚",
    "奮戦",
    "不屈",
    "逆境",
    "衆兵",
    "大勇",
    "献身",
    "扶助",
    "助勢",
    "献策",
    "雄心",
    "怪鳥",
    "炎将",
    "死王",
    "戦士",
    "腹心",
    "糧道",
    "練武",
    "統制",
    "開智",
    "人望",
    "洞察",
    "報復",
    "飛槍",
    "俊才",
    "白老",
    "戦神子",
    "伝熱",
    "不敗",
    "山読",
    "巨人",
    "才賢",
    "黒弓",
    "奇矯",
    "堅靭"
  ],
  "enishiWeights": {
    "中華統一": 8,
    "大将軍への夢": 8,
    "剣客": 6,
    "先陣をきる者": 6,
    "戦術眼": 6,
    "王の一族": 6,
    "激情家": 6,
    "忠誠心": 6,
    "弓の名手": 6,
    "政権闘争": 6,
    "槍名人": 6,
    "野心家": 6,
    "奇策奇計": 6,
    "武人": 4,
    "王騎軍": 4,
    "補佐役": 4,
    "山の民": 4,
    "経験豊富": 4,
    "暗躍": 4,
    "基本戦術": 4,
    "怪力": 4,
    "大王一派": 4,
    "真面目": 4,
    "慎重": 4,
    "謀略家": 4,
    "一騎討ち": 4,
    "飛信隊": 4,
    "荒くれ者": 4,
    "教育係": 4,
    "器用": 4,
    "将狩り": 4,
    "平穏を祈る者": 4,
    "麃公軍": 4,
    "魏軍": 4,
    "成蟜一派": 4,
    "お調子者": 4,
    "趙軍": 4,
    "後宮": 4,
    "城戸村": 4,
    "廉頗軍": 4,
    "楽華隊": 4,
    "玉鳳隊": 4,
    "王翦軍": 4,
    "燕軍": 4,
    "楚軍": 4,
    "李牧一派": 4,
    "韓軍": 4,
    "桓騎軍": 4,
    "術策": 2,
    "支援": 2,
    "防御": 2,
    "殲滅": 2,
    "主導": 2,
    "馬陽の戦い": 2,
    "王弟反乱": 2,
    "蛇甘平原の戦い": 2,
    "突撃": 2,
    "山陽の戦い": 2,
    "函谷関の戦い": 2
  },
  "sources": {
    "characterList": "https://gamewith.jp/kingdom-hadou/519260",
    "skillList": "https://gamewith.jp/kingdom-hadou/543359",
    "simulator": "https://gamewith.jp/kingdom-hadou/521187",
    "teamGuide": "https://gamewith.jp/kingdom-hadou/517327",
    "aideGuide": "https://gamewith.jp/kingdom-hadou/545445",
    "lowRarityGuide": "https://gamewith.jp/kingdom-hadou/520717",
    "officialNewbie": "https://kingdom-hadou.bn-ent.net/special/newbie/"
  }
};
